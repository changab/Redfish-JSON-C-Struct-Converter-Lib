...
//
#include <RedfishInterpreter/MetricReportCollection/EfiMetricReportCollection.h>
...
EFI_STATUS
EFIAPI
MetricReportCollectionToEfiStruct (
...
{
  EFI_REDFISH_METRICREPORTCOLLECTION *MetricReportCollection;
  EFI_REDFISH_METRICREPORTCOLLECTION_CS *MetricReportCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "MetricReportCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_MetricReportCollection_To_CS (ResoruceRaw, &MetricReportCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  MetricReportCollection = (EFI_REDFISH_METRICREPORTCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_METRICREPORTCOLLECTION));
  if (MetricReportCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)MetricReportCollection;
  MetricReportCollection->MetricReportCollection = MetricReportCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_METRICREPORTCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("MetricReportCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "MetricReportCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("MetricReportCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "MetricReportCollection");
...
EFI_STATUS
EFIAPI
MetricReportCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_MetricReportCollection_JSON (*((EFI_REDFISH_METRICREPORTCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in MetricReportCollectionToStruct.
...
EFI_STATUS
EFIAPI
MetricReportCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  MetricReportCollection = (EFI_REDFISH_METRICREPORTCOLLECTION *)InterpProp;
  DestroyMetricReportCollection_CS (MetricReportCollection->MetricReportCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from MetricReportCollectionFromEfiStruct
...
EFI_STATUS
EFIAPI
MetricReportCollectionDestoryJson(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
  IN EFI_REST_RESOURCE_TYPE_IDENTIFIER *RsrcIdentifier,  
  IN CHAR8 *ResoruceRaw
)
{
  ...
  return DestroyMetricReportCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishMetricReportCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishMetricReportCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
