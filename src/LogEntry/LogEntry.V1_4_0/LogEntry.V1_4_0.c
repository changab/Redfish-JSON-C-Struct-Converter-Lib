...
#include"Redfish_LogEntry_v1_4_0_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
static RedfishCS_status GenOemCs(RedfishLogEntry_V1_4_0_LogEntry_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst);
//
//Generate C structure for Oem
//
static RedfishCS_status GenOemActionsCs(RedfishLogEntry_V1_4_0_LogEntry_CS *Cs, json_t *JsonObj, char *Key, RedfishLogEntry_V1_4_0_OemActions_CS **Dst)
{
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishLogEntry_V1_4_0_OemActions_CS), (RedfishCS_void **)Dst);
...
//
static RedfishCS_status GenActionsCs(RedfishLogEntry_V1_4_0_LogEntry_CS *Cs, json_t *JsonObj, char *Key, RedfishLogEntry_V1_4_0_Actions_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishLogEntry_V1_4_0_Actions_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // Oem
...
//
static RedfishCS_status GenidRefCs(RedfishLogEntry_V1_4_0_LogEntry_CS *Cs, json_t *JsonObj, char *Key, Redfishodata_V4_0_3_idRef_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
...
//
static RedfishCS_status GenLinksCs(RedfishLogEntry_V1_4_0_LogEntry_CS *Cs, json_t *JsonObj, char *Key, RedfishLogEntry_V1_4_0_Links_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishLogEntry_V1_4_0_Links_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // Oem
...
//
static RedfishCS_status GenRedfishCS_char_ArrayCs(RedfishLogEntry_V1_4_0_LogEntry_CS *Cs, json_t *JsonObj, char *Key, RedfishCS_char_Array **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
...
static RedfishCS_status CS_To_JSON_ActionsOem(json_t *CsJson, char *Key, RedfishLogEntry_V1_4_0_OemActions_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
static RedfishCS_status CS_To_JSON_Actions(json_t *CsJson, char *Key, RedfishLogEntry_V1_4_0_Actions_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
static RedfishCS_status CS_To_JSON_LinksOriginOfCondition(json_t *CsJson, char *Key, Redfishodata_V4_0_3_idRef_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
static RedfishCS_status CS_To_JSON_Links(json_t *CsJson, char *Key, RedfishLogEntry_V1_4_0_Links_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
  ...
  // OriginOfCondition
  if (CS_To_JSON_LinksOriginOfCondition(CsJson, "OriginOfCondition", CSPtr->OriginOfCondition) != RedfishCS_status_success) {goto Error;}
...
//
// C structure to JSON for LogEntry.V1_4_0.LogEntry
//
RedfishCS_status CS_To_LogEntry_V1_4_0_JSON(RedfishLogEntry_V1_4_0_LogEntry_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
  ...
  // Created 
  if (InsertJsonStringObj (CsJson, "Created", CSPtr->Created) != RedfishCS_status_success) {goto Error;}

  // Description 
  if (InsertJsonStringObj (CsJson, "Description", CSPtr->Description) != RedfishCS_status_success) {goto Error;}

  // EntryCode 
  if (InsertJsonStringObj (CsJson, "EntryCode", CSPtr->EntryCode) != RedfishCS_status_success) {goto Error;}

  // EntryType 
  if (InsertJsonStringObj (CsJson, "EntryType", CSPtr->EntryType) != RedfishCS_status_success) {goto Error;}

  // EventGroupId 
  if (InsertJsonInt64Obj (CsJson, "EventGroupId", CSPtr->EventGroupId) != RedfishCS_status_success) {goto Error;}

  // EventId 
  if (InsertJsonStringObj (CsJson, "EventId", CSPtr->EventId) != RedfishCS_status_success) {goto Error;}

  // EventTimestamp 
  if (InsertJsonStringObj (CsJson, "EventTimestamp", CSPtr->EventTimestamp) != RedfishCS_status_success) {goto Error;}

  // EventType 
  if (InsertJsonStringObj (CsJson, "EventType", CSPtr->EventType) != RedfishCS_status_success) {goto Error;}
  ...
  // MessageArgs 
  if (InsertJsonStringArrayObj (CsJson, "MessageArgs", CSPtr->MessageArgs) != RedfishCS_status_success) {goto Error;}

  // MessageId 
  if (InsertJsonStringObj (CsJson, "MessageId", CSPtr->MessageId) != RedfishCS_status_success) {goto Error;}
  ...
  // OemLogEntryCode 
  if (InsertJsonStringObj (CsJson, "OemLogEntryCode", CSPtr->OemLogEntryCode) != RedfishCS_status_success) {goto Error;}

  // OemRecordFormat 
  if (InsertJsonStringObj (CsJson, "OemRecordFormat", CSPtr->OemRecordFormat) != RedfishCS_status_success) {goto Error;}

  // OemSensorType 
  if (InsertJsonStringObj (CsJson, "OemSensorType", CSPtr->OemSensorType) != RedfishCS_status_success) {goto Error;}

  // SensorNumber 
  if (InsertJsonInt64Obj (CsJson, "SensorNumber", CSPtr->SensorNumber) != RedfishCS_status_success) {goto Error;}

  // SensorType 
  if (InsertJsonStringObj (CsJson, "SensorType", CSPtr->SensorType) != RedfishCS_status_success) {goto Error;}

  // Severity 
  if (InsertJsonStringObj (CsJson, "Severity", CSPtr->Severity) != RedfishCS_status_success) {goto Error;}
...
//
// Destory C Structure for LogEntry.V1_4_0.LogEntry
//
RedfishCS_status DestroyLogEntry_V1_4_0_CS(RedfishLogEntry_V1_4_0_LogEntry_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for LogEntry.V1_4_0.LogEntry
//
RedfishCS_status DestroyLogEntry_V1_4_0_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for LogEntry.V1_4_0.LogEntry
//
RedfishCS_status
Json_LogEntry_V1_4_0_To_CS(RedfishCS_char *JsonRawText, RedfishLogEntry_V1_4_0_LogEntry_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishLogEntry_V1_4_0_LogEntry_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "LogEntry", "v1_4_0", "LogEntry", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishLogEntry_V1_4_0_LogEntry_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
  ...
  // Created
  Status = GetRedfishPropertyStr (Cs, JsonObj, "Created", &Cs->Created);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
  ...
  // EntryCode
  Status = GetRedfishPropertyStr (Cs, JsonObj, "EntryCode", &Cs->EntryCode);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // EntryType
  Status = GetRedfishPropertyStr (Cs, JsonObj, "EntryType", &Cs->EntryType);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){goto Error;}}

  // EventGroupId
  Status = GetRedfishPropertyInt64 (Cs, JsonObj, "EventGroupId", &Cs->EventGroupId);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // EventId
  Status = GetRedfishPropertyStr (Cs, JsonObj, "EventId", &Cs->EventId);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // EventTimestamp
  Status = GetRedfishPropertyStr (Cs, JsonObj, "EventTimestamp", &Cs->EventTimestamp);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // EventType
  Status = GetRedfishPropertyStr (Cs, JsonObj, "EventType", &Cs->EventType);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
  ...
  // Message
  Status = GetRedfishPropertyStr (Cs, JsonObj, "Message", &Cs->Message);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // MessageArgs
  Status = GenRedfishCS_char_ArrayCs (Cs, JsonObj, "MessageArgs", &Cs->MessageArgs);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // MessageId
  Status = GetRedfishPropertyStr (Cs, JsonObj, "MessageId", &Cs->MessageId);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
  ...
  // OemLogEntryCode
  Status = GetRedfishPropertyStr (Cs, JsonObj, "OemLogEntryCode", &Cs->OemLogEntryCode);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // OemRecordFormat
  Status = GetRedfishPropertyStr (Cs, JsonObj, "OemRecordFormat", &Cs->OemRecordFormat);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // OemSensorType
  Status = GetRedfishPropertyStr (Cs, JsonObj, "OemSensorType", &Cs->OemSensorType);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // SensorNumber
  Status = GetRedfishPropertyInt64 (Cs, JsonObj, "SensorNumber", &Cs->SensorNumber);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // SensorType
  Status = GetRedfishPropertyStr (Cs, JsonObj, "SensorType", &Cs->SensorType);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // Severity
  Status = GetRedfishPropertyStr (Cs, JsonObj, "Severity", &Cs->Severity);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
