...
//
#include <RedfishInterpreter/ComputerSystemCollection/EfiComputerSystemCollection.h>
CHAR8 mResourceTypeStr [] = "#ComputerSystemCollection";
BOOLEAN IsRevisonController = FALSE;

// Support ComputerSystemCollection  
...
EFI_STATUS
EFIAPI
ComputerSystemCollectionToEfiStruct (
...
{
  EFI_REDFISH_COMPUTERSYSTEMCOLLECTION *ComputerSystemCollection;
  EFI_REDFISH_COMPUTERSYSTEMCOLLECTION_CS *ComputerSystemCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "ComputerSystemCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_ComputerSystemCollection_To_CS (ResoruceRaw, &ComputerSystemCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ComputerSystemCollection = (EFI_REDFISH_COMPUTERSYSTEMCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_COMPUTERSYSTEMCOLLECTION));
  if (ComputerSystemCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)ComputerSystemCollection;
  ...
  Length = sizeof (EFI_REDFISH_COMPUTERSYSTEMCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("ComputerSystemCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "ComputerSystemCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("ComputerSystemCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "ComputerSystemCollection");
...
EFI_STATUS
EFIAPI
ComputerSystemCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_ComputerSystemCollection_JSON (*((EFI_REDFISH_COMPUTERSYSTEMCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in ComputerSystemCollectionToStruct.
...
EFI_STATUS
EFIAPI
ComputerSystemCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  ComputerSystemCollection = (EFI_REDFISH_COMPUTERSYSTEMCOLLECTION *)InterpProp;
  DestroyComputerSystemCollection_CS (ComputerSystemCollection->ComputerSystemCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from ComputerSystemCollectionFromEfiStruct
...
EFI_STATUS
EFIAPI
ComputerSystemCollectionDestoryJson(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
  IN EFI_REST_RESOURCE_TYPE_IDENTIFIER *RsrcIdentifier,  
  IN CHAR8 *ResoruceRaw
)
{
  ...
  return DestroyComputerSystemCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishComputerSystemCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishComputerSystemCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
