...
//
#include <RedfishInterpreter/VolumeCollection/EfiVolumeCollection.h>
CHAR8 mResourceTypeStr [] = "#VolumeCollection";
...
{
  EFI_REDFISH_VOLUMECOLLECTION *VolumeCollection;
  EFI_REDFISH_VOLUMECOLLECTION_CS *VolumeCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "VolumeCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_VolumeCollection_To_CS (ResoruceRaw, &VolumeCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  VolumeCollection = (EFI_REDFISH_VOLUMECOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_VOLUMECOLLECTION));
  if (VolumeCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)VolumeCollection;
  VolumeCollection->VolumeCollection = VolumeCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_VOLUMECOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("VolumeCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "VolumeCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("VolumeCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "VolumeCollection");
...
)
{
  ...
  return CS_To_VolumeCollection_JSON (*((EFI_REDFISH_VOLUMECOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in VolumeCollectionToStruct.
...
EFI_STATUS
EFIAPI
VolumeCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  VolumeCollection = (EFI_REDFISH_VOLUMECOLLECTION *)InterpProp;
  DestroyVolumeCollection_CS (VolumeCollection->VolumeCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from VolumeCollectionFromEfiStruct
...
)
{
  ...
  return DestroyVolumeCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishVolumeCollectionEntryPoint (
