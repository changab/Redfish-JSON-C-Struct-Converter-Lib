...
//
#include <RedfishInterpreter/MemoryCollection/EfiMemoryCollection.h>
CHAR8 mResourceTypeStr [] = "#MemoryCollection";
...
{
  EFI_REDFISH_MEMORYCOLLECTION *MemoryCollection;
  EFI_REDFISH_MEMORYCOLLECTION_CS *MemoryCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "MemoryCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_MemoryCollection_To_CS (ResoruceRaw, &MemoryCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  MemoryCollection = (EFI_REDFISH_MEMORYCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_MEMORYCOLLECTION));
  if (MemoryCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)MemoryCollection;
  MemoryCollection->MemoryCollection = MemoryCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_MEMORYCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("MemoryCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "MemoryCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("MemoryCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "MemoryCollection");
...
)
{
  ...
  return CS_To_MemoryCollection_JSON (*((EFI_REDFISH_MEMORYCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in MemoryCollectionToStruct.
...
EFI_STATUS
EFIAPI
MemoryCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  MemoryCollection = (EFI_REDFISH_MEMORYCOLLECTION *)InterpProp;
  DestroyMemoryCollection_CS (MemoryCollection->MemoryCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from MemoryCollectionFromEfiStruct
...
)
{
  ...
  return DestroyMemoryCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishMemoryCollectionEntryPoint (
