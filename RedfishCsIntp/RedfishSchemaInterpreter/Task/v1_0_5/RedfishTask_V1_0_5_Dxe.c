...
//
#include <RedfishInterpreter/Task/v1_0_5/EfiTaskV1_0_5.h>
CHAR8 mResourceTypeStr [] = "#Task";
...
{
  EFI_REDFISH_TASK_V1_0_5 *TaskV1_0_5;
  EFI_REDFISH_TASK_V1_0_5_CS *TaskV1_0_5Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "Task") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_Task_V1_0_5_To_CS (ResoruceRaw, &TaskV1_0_5Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  TaskV1_0_5 = (EFI_REDFISH_TASK_V1_0_5 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_TASK_V1_0_5));
  if (TaskV1_0_5 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)TaskV1_0_5;
  TaskV1_0_5->Task = TaskV1_0_5Cs;
  ResourceTypeMajorVersion = "1";
  ResourceTypeMinorVersion = "0";
  ResourceTypeErrataVersion = "5";
  Length = sizeof (EFI_REDFISH_TASK_V1_0_5_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("Task"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "Task");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("Task"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "Task");
...
)
{
  ...
  return CS_To_Task_V1_0_5_JSON (*((EFI_REDFISH_TASK_V1_0_5_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in TaskToStruct.
...
)
{
  ...
  TaskV1_0_5 = (EFI_REDFISH_TASK_V1_0_5 *)InterpProp;
  DestroyTask_V1_0_5_CS (TaskV1_0_5->Task); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from TaskFromEfiStruct
...
)
{
  ...
  return DestroyTask_V1_0_5_Json (ResoruceRaw);
}
