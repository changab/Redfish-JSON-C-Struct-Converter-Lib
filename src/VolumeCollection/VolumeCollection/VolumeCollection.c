...
#include"Redfish_VolumeCollection_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
//
static RedfishCS_status GenOemCs(RedfishVolumeCollection_VolumeCollection_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst)
{
...
//
// C structure to JSON for VolumeCollection.VolumeCollection
//
RedfishCS_status CS_To_VolumeCollection_JSON(RedfishVolumeCollection_VolumeCollection_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
  ...
  if (InsertJsonStringObj (CsJson, "Members@odata.navigationLink", CSPtr->Membersodata_navigationLink) != RedfishCS_status_success) {goto Error;}
...
//
// Destory C Structure for VolumeCollection.VolumeCollection
//
RedfishCS_status DestroyVolumeCollection_CS(RedfishVolumeCollection_VolumeCollection_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for VolumeCollection.VolumeCollection
//
RedfishCS_status DestroyVolumeCollection_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for VolumeCollection.NOVERSIONED.VolumeCollection
//
RedfishCS_status
Json_VolumeCollection_To_CS(RedfishCS_char *JsonRawText, RedfishVolumeCollection_VolumeCollection_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishVolumeCollection_VolumeCollection_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "VolumeCollection", "noversioned", "VolumeCollection", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishVolumeCollection_VolumeCollection_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
  ...
  // Members@odata.navigationLink
  Status = GetRedfishPropertyStr (Cs, JsonObj, "Members@odata.navigationLink", &Cs->Membersodata_navigationLink);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
