...
//
#include <RedfishInterpreter/TaskCollection/EfiTaskCollection.h>
CHAR8 mResourceTypeStr [] = "#TaskCollection";
...
{
  EFI_REDFISH_TASKCOLLECTION *TaskCollection;
  EFI_REDFISH_TASKCOLLECTION_CS *TaskCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "TaskCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_TaskCollection_To_CS (ResoruceRaw, &TaskCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  TaskCollection = (EFI_REDFISH_TASKCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_TASKCOLLECTION));
  if (TaskCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)TaskCollection;
  TaskCollection->TaskCollection = TaskCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_TASKCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("TaskCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "TaskCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("TaskCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "TaskCollection");
...
)
{
  ...
  return CS_To_TaskCollection_JSON (*((EFI_REDFISH_TASKCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in TaskCollectionToStruct.
...
EFI_STATUS
EFIAPI
TaskCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  TaskCollection = (EFI_REDFISH_TASKCOLLECTION *)InterpProp;
  DestroyTaskCollection_CS (TaskCollection->TaskCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from TaskCollectionFromEfiStruct
...
)
{
  ...
  return DestroyTaskCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishTaskCollectionEntryPoint (
