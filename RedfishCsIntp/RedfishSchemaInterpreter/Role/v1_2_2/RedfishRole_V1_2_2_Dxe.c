...
//
#include <RedfishInterpreter/Role/v1_2_2/EfiRoleV1_2_2.h>
CHAR8 mResourceTypeStr [] = "#Role";
...
{
  EFI_REDFISH_ROLE_V1_2_2 *RoleV1_2_2;
  EFI_REDFISH_ROLE_V1_2_2_CS *RoleV1_2_2Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "Role") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_Role_V1_2_2_To_CS (ResoruceRaw, &RoleV1_2_2Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  RoleV1_2_2 = (EFI_REDFISH_ROLE_V1_2_2 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_ROLE_V1_2_2));
  if (RoleV1_2_2 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)RoleV1_2_2;
  RoleV1_2_2->Role = RoleV1_2_2Cs;
  ResourceTypeMajorVersion = "1";
  ResourceTypeMinorVersion = "2";
  ResourceTypeErrataVersion = "2";
  Length = sizeof (EFI_REDFISH_ROLE_V1_2_2_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("Role"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "Role");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("Role"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "Role");
...
)
{
  ...
  return CS_To_Role_V1_2_2_JSON (*((EFI_REDFISH_ROLE_V1_2_2_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in RoleToStruct.
...
)
{
  ...
  RoleV1_2_2 = (EFI_REDFISH_ROLE_V1_2_2 *)InterpProp;
  DestroyRole_V1_2_2_CS (RoleV1_2_2->Role); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from RoleFromEfiStruct
...
)
{
  ...
  return DestroyRole_V1_2_2_Json (ResoruceRaw);
}
