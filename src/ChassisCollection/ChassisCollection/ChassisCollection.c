...
#include"Redfish_ChassisCollection_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
//
static RedfishCS_status GenOemCs(RedfishChassisCollection_ChassisCollection_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst)
{
...
//
// C structure to JSON for ChassisCollection.ChassisCollection
//
RedfishCS_status CS_To_ChassisCollection_JSON(RedfishChassisCollection_ChassisCollection_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
...
//
// Destory C Structure for ChassisCollection.ChassisCollection
//
RedfishCS_status DestroyChassisCollection_CS(RedfishChassisCollection_ChassisCollection_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for ChassisCollection.ChassisCollection
//
RedfishCS_status DestroyChassisCollection_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for ChassisCollection.NOVERSIONED.ChassisCollection
//
RedfishCS_status
Json_ChassisCollection_To_CS(RedfishCS_char *JsonRawText, RedfishChassisCollection_ChassisCollection_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishChassisCollection_ChassisCollection_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "ChassisCollection", "noversioned", "ChassisCollection", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishChassisCollection_ChassisCollection_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
...
Error:;
  json_decref(JsonObj);
  DestroyChassisCollection_CS (Cs);
  return Status;
}
