...
//
#include <RedfishInterpreter/VLanNetworkInterfaceCollection/EfiVLanNetworkInterfaceCollection.h>
CHAR8 mResourceTypeStr [] = "#VLanNetworkInterfaceCollection";
BOOLEAN IsRevisonController = FALSE;

// Support VLanNetworkInterfaceCollection  
...
EFI_STATUS
EFIAPI
VLanNetworkInterfaceCollectionToEfiStruct (
...
{
  EFI_REDFISH_VLANNETWORKINTERFACECOLLECTION *VLanNetworkInterfaceCollection;
  EFI_REDFISH_VLANNETWORKINTERFACECOLLECTION_CS *VLanNetworkInterfaceCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "VLanNetworkInterfaceCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_VLanNetworkInterfaceCollection_To_CS (ResoruceRaw, &VLanNetworkInterfaceCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  VLanNetworkInterfaceCollection = (EFI_REDFISH_VLANNETWORKINTERFACECOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_VLANNETWORKINTERFACECOLLECTION));
  if (VLanNetworkInterfaceCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)VLanNetworkInterfaceCollection;
  VLanNetworkInterfaceCollection->VLanNetworkInterfaceCollection = VLanNetworkInterfaceCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_VLANNETWORKINTERFACECOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("VLanNetworkInterfaceCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "VLanNetworkInterfaceCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("VLanNetworkInterfaceCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "VLanNetworkInterfaceCollection");
...
EFI_STATUS
EFIAPI
VLanNetworkInterfaceCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_VLanNetworkInterfaceCollection_JSON (*((EFI_REDFISH_VLANNETWORKINTERFACECOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in VLanNetworkInterfaceCollectionToStruct.
...
EFI_STATUS
EFIAPI
VLanNetworkInterfaceCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  VLanNetworkInterfaceCollection = (EFI_REDFISH_VLANNETWORKINTERFACECOLLECTION *)InterpProp;
  DestroyVLanNetworkInterfaceCollection_CS (VLanNetworkInterfaceCollection->VLanNetworkInterfaceCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from VLanNetworkInterfaceCollectionFromEfiStruct
...
EFI_STATUS
EFIAPI
VLanNetworkInterfaceCollectionDestoryJson(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
  IN EFI_REST_RESOURCE_TYPE_IDENTIFIER *RsrcIdentifier,  
  IN CHAR8 *ResoruceRaw
)
{
  ...
  return DestroyVLanNetworkInterfaceCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishVLanNetworkInterfaceCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishVLanNetworkInterfaceCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
