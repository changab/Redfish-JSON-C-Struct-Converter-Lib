...
//
#include <RedfishInterpreter/SessionCollection/EfiSessionCollection.h>
CHAR8 mResourceTypeStr [] = "#SessionCollection";
BOOLEAN IsRevisonController = FALSE;

// Support SessionCollection  
...
{
  EFI_REDFISH_SESSIONCOLLECTION *SessionCollection;
  EFI_REDFISH_SESSIONCOLLECTION_CS *SessionCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "SessionCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_SessionCollection_To_CS (ResoruceRaw, &SessionCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  SessionCollection = (EFI_REDFISH_SESSIONCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_SESSIONCOLLECTION));
  if (SessionCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)SessionCollection;
  SessionCollection->SessionCollection = SessionCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_SESSIONCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("SessionCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "SessionCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("SessionCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "SessionCollection");
...
EFI_STATUS
EFIAPI
SessionCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_SessionCollection_JSON (*((EFI_REDFISH_SESSIONCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in SessionCollectionToStruct.
...
EFI_STATUS
EFIAPI
SessionCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  SessionCollection = (EFI_REDFISH_SESSIONCOLLECTION *)InterpProp;
  DestroySessionCollection_CS (SessionCollection->SessionCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from SessionCollectionFromEfiStruct
...
)
{
  ...
  return DestroySessionCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishSessionCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishSessionCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
