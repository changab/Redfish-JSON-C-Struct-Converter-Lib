...
//
#include <RedfishInterpreter/SwitchCollection/EfiSwitchCollection.h>
CHAR8 mResourceTypeStr [] = "#SwitchCollection";
...
{
  EFI_REDFISH_SWITCHCOLLECTION *SwitchCollection;
  EFI_REDFISH_SWITCHCOLLECTION_CS *SwitchCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "SwitchCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_SwitchCollection_To_CS (ResoruceRaw, &SwitchCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  SwitchCollection = (EFI_REDFISH_SWITCHCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_SWITCHCOLLECTION));
  if (SwitchCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)SwitchCollection;
  SwitchCollection->SwitchCollection = SwitchCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_SWITCHCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("SwitchCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "SwitchCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("SwitchCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "SwitchCollection");
...
)
{
  ...
  return CS_To_SwitchCollection_JSON (*((EFI_REDFISH_SWITCHCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in SwitchCollectionToStruct.
...
EFI_STATUS
EFIAPI
SwitchCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  SwitchCollection = (EFI_REDFISH_SWITCHCOLLECTION *)InterpProp;
  DestroySwitchCollection_CS (SwitchCollection->SwitchCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from SwitchCollectionFromEfiStruct
...
)
{
  ...
  return DestroySwitchCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishSwitchCollectionEntryPoint (
