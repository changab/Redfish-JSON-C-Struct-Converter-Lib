...
//
#include <RedfishInterpreter/MemoryDomainCollection/EfiMemoryDomainCollection.h>
CHAR8 mResourceTypeStr [] = "#MemoryDomainCollection";
BOOLEAN IsRevisonController = FALSE;

// Support MemoryDomainCollection  
...
EFI_STATUS
EFIAPI
MemoryDomainCollectionToEfiStruct (
...
{
  EFI_REDFISH_MEMORYDOMAINCOLLECTION *MemoryDomainCollection;
  EFI_REDFISH_MEMORYDOMAINCOLLECTION_CS *MemoryDomainCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "MemoryDomainCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_MemoryDomainCollection_To_CS (ResoruceRaw, &MemoryDomainCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  MemoryDomainCollection = (EFI_REDFISH_MEMORYDOMAINCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_MEMORYDOMAINCOLLECTION));
  if (MemoryDomainCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)MemoryDomainCollection;
  MemoryDomainCollection->MemoryDomainCollection = MemoryDomainCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_MEMORYDOMAINCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("MemoryDomainCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "MemoryDomainCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("MemoryDomainCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "MemoryDomainCollection");
...
EFI_STATUS
EFIAPI
MemoryDomainCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_MemoryDomainCollection_JSON (*((EFI_REDFISH_MEMORYDOMAINCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in MemoryDomainCollectionToStruct.
...
EFI_STATUS
EFIAPI
MemoryDomainCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  MemoryDomainCollection = (EFI_REDFISH_MEMORYDOMAINCOLLECTION *)InterpProp;
  DestroyMemoryDomainCollection_CS (MemoryDomainCollection->MemoryDomainCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from MemoryDomainCollectionFromEfiStruct
...
EFI_STATUS
EFIAPI
MemoryDomainCollectionDestoryJson(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
  IN EFI_REST_RESOURCE_TYPE_IDENTIFIER *RsrcIdentifier,  
  IN CHAR8 *ResoruceRaw
)
{
  ...
  return DestroyMemoryDomainCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishMemoryDomainCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishMemoryDomainCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
