...
//
#include <RedfishInterpreter/ManagerAccountCollection/EfiManagerAccountCollection.h>
CHAR8 mResourceTypeStr [] = "#ManagerAccountCollection";
BOOLEAN IsRevisonController = FALSE;

// Support ManagerAccountCollection  
...
EFI_STATUS
EFIAPI
ManagerAccountCollectionToEfiStruct (
...
{
  EFI_REDFISH_MANAGERACCOUNTCOLLECTION *ManagerAccountCollection;
  EFI_REDFISH_MANAGERACCOUNTCOLLECTION_CS *ManagerAccountCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "ManagerAccountCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_ManagerAccountCollection_To_CS (ResoruceRaw, &ManagerAccountCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ManagerAccountCollection = (EFI_REDFISH_MANAGERACCOUNTCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_MANAGERACCOUNTCOLLECTION));
  if (ManagerAccountCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)ManagerAccountCollection;
  ManagerAccountCollection->ManagerAccountCollection = ManagerAccountCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_MANAGERACCOUNTCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("ManagerAccountCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "ManagerAccountCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("ManagerAccountCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "ManagerAccountCollection");
...
EFI_STATUS
EFIAPI
ManagerAccountCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_ManagerAccountCollection_JSON (*((EFI_REDFISH_MANAGERACCOUNTCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in ManagerAccountCollectionToStruct.
...
EFI_STATUS
EFIAPI
ManagerAccountCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  ManagerAccountCollection = (EFI_REDFISH_MANAGERACCOUNTCOLLECTION *)InterpProp;
  DestroyManagerAccountCollection_CS (ManagerAccountCollection->ManagerAccountCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from ManagerAccountCollectionFromEfiStruct
...
EFI_STATUS
EFIAPI
ManagerAccountCollectionDestoryJson(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
  IN EFI_REST_RESOURCE_TYPE_IDENTIFIER *RsrcIdentifier,  
  IN CHAR8 *ResoruceRaw
)
{
  ...
  return DestroyManagerAccountCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishManagerAccountCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishManagerAccountCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
