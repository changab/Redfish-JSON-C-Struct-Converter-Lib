...
#include"Redfish_Assembly_v1_1_1_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
static RedfishCS_status GenOemCs(RedfishAssembly_V1_1_1_Assembly_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst);
//
//Generate C structure for Oem
//
static RedfishCS_status GenOemActionsCs(RedfishAssembly_V1_1_1_Assembly_CS *Cs, json_t *JsonObj, char *Key, RedfishAssembly_V1_1_1_OemActions_CS **Dst)
{
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishAssembly_V1_1_1_OemActions_CS), (RedfishCS_void **)Dst);
...
//
static RedfishCS_status GenActionsCs(RedfishAssembly_V1_1_1_Assembly_CS *Cs, json_t *JsonObj, char *Key, RedfishAssembly_V1_1_1_Actions_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishAssembly_V1_1_1_Actions_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // Oem
...
//Generate C structure for Oem
//
static RedfishCS_status GenAssemblyDataOemActionsCs(RedfishAssembly_V1_1_1_Assembly_CS *Cs, json_t *JsonObj, char *Key, RedfishAssembly_V1_1_1_AssemblyDataOemActions_CS **Dst)
{
  RedfishCS_status Status;
  RedfishCS_Type_JSON_Data *CsTypeJson;
  RedfishCS_Type_EmptyProp_CS_Data *CsTypeEmptyPropCS;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishAssembly_V1_1_1_AssemblyDataOemActions_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  InitializeLinkHead (&(*Dst)->Prop);
...
//Generate C structure for Actions
//
static RedfishCS_status GenAssemblyDataActionsCs(RedfishAssembly_V1_1_1_Assembly_CS *Cs, json_t *JsonObj, char *Key, RedfishAssembly_V1_1_1_AssemblyDataActions_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishAssembly_V1_1_1_AssemblyDataActions_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // Oem
  Status = GenAssemblyDataOemActionsCs (Cs, TempJsonObj, "Oem", &(*Dst)->Oem);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
...
//
static RedfishCS_status GenStatusCs(RedfishAssembly_V1_1_1_Assembly_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Status_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
...
static RedfishCS_status GenAssemblyData_Array_ElementCs(RedfishAssembly_V1_1_1_Assembly_CS *Cs, json_t *JsonObj, RedfishCS_uint64 Index,  RedfishAssembly_V1_1_1_AssemblyData_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsZeroMemory(Cs, sizeof(RedfishAssembly_V1_1_1_AssemblyData_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // @odata.id
  ...
  // Actions
  Status = GenAssemblyDataActionsCs (Cs, TempJsonObj, "Actions", &(*Dst)->Actions);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // BinaryDataURI
  Status = GetRedfishPropertyStr (Cs, TempJsonObj, "BinaryDataURI", &(*Dst)->BinaryDataURI);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // Description
  Status = GetRedfishPropertyStr (Cs, TempJsonObj, "Description", &(*Dst)->Description);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // EngineeringChangeLevel
  Status = GetRedfishPropertyStr (Cs, TempJsonObj, "EngineeringChangeLevel", &(*Dst)->EngineeringChangeLevel);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
  ...
  // Producer
  Status = GetRedfishPropertyStr (Cs, TempJsonObj, "Producer", &(*Dst)->Producer);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // ProductionDate
  Status = GetRedfishPropertyStr (Cs, TempJsonObj, "ProductionDate", &(*Dst)->ProductionDate);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
  ...
  // Vendor
  Status = GetRedfishPropertyStr (Cs, TempJsonObj, "Vendor", &(*Dst)->Vendor);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // Version
  Status = GetRedfishPropertyStr (Cs, TempJsonObj, "Version", &(*Dst)->Version);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
...
//
//Generate C structure for RedfishAssembly_V1_1_1_AssemblyData_Array_CS
//
static RedfishCS_status GenAssemblyData_ArrayCs(RedfishAssembly_V1_1_1_Assembly_CS *Cs, json_t *JsonObj, char *Key, RedfishAssembly_V1_1_1_AssemblyData_Array_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  RedfishCS_uint64 ArraySize;
  RedfishCS_uint64 Index;
  RedfishAssembly_V1_1_1_AssemblyData_Array_CS *ThisElement;
  ...
  Status = allocateArrayRecordCsMemory(Cs, sizeof (RedfishAssembly_V1_1_1_AssemblyData_Array_CS), ArraySize, (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  if (*Dst == NULL) {
  ...
  for (Index = 0; Index < ArraySize; Index ++) {
    Status = GenAssemblyData_Array_ElementCs(Cs, TempJsonObj, Index, &ThisElement->ArrayValue);
    if (Status != RedfishCS_status_success){
      goto Error;
    }
...
static RedfishCS_status CS_To_JSON_ActionsOem(json_t *CsJson, char *Key, RedfishAssembly_V1_1_1_OemActions_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
static RedfishCS_status CS_To_JSON_Actions(json_t *CsJson, char *Key, RedfishAssembly_V1_1_1_Actions_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
static RedfishCS_status CS_To_JSON_AssembliesActionsOem(json_t *CsJson, char *Key, RedfishAssembly_V1_1_1_AssemblyDataOemActions_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
static RedfishCS_status CS_To_JSON_AssembliesActions(json_t *CsJson, char *Key, RedfishAssembly_V1_1_1_AssemblyDataActions_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
  // Oem
  if (CS_To_JSON_AssembliesActionsOem(CsJson, "Oem", CSPtr->Oem) != RedfishCS_status_success) {goto Error;}

  return RedfishCS_status_success;
Error:;
  return RedfishCS_status_unsupported;
}
static RedfishCS_status CS_To_JSON_AssembliesOem(json_t *CsJson, char *Key, RedfishResource_Oem_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
static RedfishCS_status CS_To_JSON_AssembliesStatusOem(json_t *CsJson, char *Key, RedfishResource_Oem_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
static RedfishCS_status CS_To_JSON_AssembliesStatus(json_t *CsJson, char *Key, RedfishResource_Status_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
  ...
  // Oem
  if (CS_To_JSON_AssembliesStatusOem(CsJson, "Oem", CSPtr->Oem) != RedfishCS_status_success) {goto Error;}
...
static RedfishCS_status CS_To_JSON_Assemblies(json_t *CsJson, char *Key, RedfishAssembly_V1_1_1_AssemblyData_Array_CS *CSPtr)
{
  json_t *ArrayJson;
  json_t *ArrayMember;
  RedfishAssembly_V1_1_1_AssemblyData_Array_CS *NextArray;
  RedfishAssembly_V1_1_1_AssemblyData_CS *NextArrayItem;
  ...
  do {
    ...
    // Actions
    if (CS_To_JSON_AssembliesActions(ArrayMember, "Actions", NextArrayItem->Actions) != RedfishCS_status_success) {goto Error;}

    // BinaryDataURI 
    if (InsertJsonStringObj (ArrayMember, "BinaryDataURI", NextArrayItem->BinaryDataURI) != RedfishCS_status_success) {goto Error;}

    // Description 
    if (InsertJsonStringObj (ArrayMember, "Description", NextArrayItem->Description) != RedfishCS_status_success) {goto Error;}

    // EngineeringChangeLevel 
    if (InsertJsonStringObj (ArrayMember, "EngineeringChangeLevel", NextArrayItem->EngineeringChangeLevel) != RedfishCS_status_success) {goto Error;}
    ...
    // Oem
    if (CS_To_JSON_AssembliesOem(ArrayMember, "Oem", NextArrayItem->Oem) != RedfishCS_status_success) {goto Error;}

    // PartNumber 
    if (InsertJsonStringObj (ArrayMember, "PartNumber", NextArrayItem->PartNumber) != RedfishCS_status_success) {goto Error;}

    // Producer 
    if (InsertJsonStringObj (ArrayMember, "Producer", NextArrayItem->Producer) != RedfishCS_status_success) {goto Error;}

    // ProductionDate 
    if (InsertJsonStringObj (ArrayMember, "ProductionDate", NextArrayItem->ProductionDate) != RedfishCS_status_success) {goto Error;}
    ...
    // Status
    if (CS_To_JSON_AssembliesStatus(ArrayMember, "Status", NextArrayItem->Status) != RedfishCS_status_success) {goto Error;}

    // Vendor 
    if (InsertJsonStringObj (ArrayMember, "Vendor", NextArrayItem->Vendor) != RedfishCS_status_success) {goto Error;}

    // Version 
    if (InsertJsonStringObj (ArrayMember, "Version", NextArrayItem->Version) != RedfishCS_status_success) {goto Error;}
...
//
// C structure to JSON for Assembly.V1_1_1.Assembly
//
RedfishCS_status CS_To_Assembly_V1_1_1_JSON(RedfishAssembly_V1_1_1_Assembly_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
  ...
  // Assemblies
  if (CS_To_JSON_Assemblies(CsJson, "Assemblies", CSPtr->Assemblies) != RedfishCS_status_success) {goto Error;}

  // Assemblies@odata.count 
  if (InsertJsonInt64Obj (CsJson, "Assemblies@odata.count", CSPtr->Assembliesodata_count) != RedfishCS_status_success) {goto Error;}
...
//
// Destory C Structure for Assembly.V1_1_1.Assembly
//
RedfishCS_status DestroyAssembly_V1_1_1_CS(RedfishAssembly_V1_1_1_Assembly_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for Assembly.V1_1_1.Assembly
//
RedfishCS_status DestroyAssembly_V1_1_1_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for Assembly.V1_1_1.Assembly
//
RedfishCS_status
Json_Assembly_V1_1_1_To_CS(RedfishCS_char *JsonRawText, RedfishAssembly_V1_1_1_Assembly_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishAssembly_V1_1_1_Assembly_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "Assembly", "v1_1_1", "Assembly", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishAssembly_V1_1_1_Assembly_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
  ...
  // Assemblies
  Status = GenAssemblyData_ArrayCs (Cs, JsonObj, "Assemblies", &Cs->Assemblies);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // Assemblies@odata.count
  Status = GetRedfishPropertyInt64 (Cs, JsonObj, "Assemblies@odata.count", &Cs->Assembliesodata_count);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
