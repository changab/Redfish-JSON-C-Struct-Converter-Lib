...
//
#include <RedfishInterpreter/MemoryChunks/v1_2_0/EfiMemoryChunksV1_2_0.h>
CHAR8 mResourceTypeStr [] = "#MemoryChunks";
BOOLEAN IsRevisonController = TRUE;

// Support MemoryChunks V1_2_0 
...
{
  EFI_REDFISH_MEMORYCHUNKS_V1_2_0 *MemoryChunksV1_2_0;
  EFI_REDFISH_MEMORYCHUNKS_V1_2_0_CS *MemoryChunksV1_2_0Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "MemoryChunks") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_MemoryChunks_V1_2_0_To_CS (ResoruceRaw, &MemoryChunksV1_2_0Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  MemoryChunksV1_2_0 = (EFI_REDFISH_MEMORYCHUNKS_V1_2_0 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_MEMORYCHUNKS_V1_2_0));
  if (MemoryChunksV1_2_0 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)MemoryChunksV1_2_0;
  MemoryChunksV1_2_0->MemoryChunks = MemoryChunksV1_2_0Cs;
  ResourceTypeMajorVersion = "1";
  ResourceTypeMinorVersion = "2";
  ResourceTypeErrataVersion = "0";
  Length = sizeof (EFI_REDFISH_MEMORYCHUNKS_V1_2_0_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("MemoryChunks"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "MemoryChunks");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("MemoryChunks"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "MemoryChunks");
...
)
{
  ...
  return CS_To_MemoryChunks_V1_2_0_JSON (*((EFI_REDFISH_MEMORYCHUNKS_V1_2_0_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in MemoryChunksToStruct.
...
)
{
  ...
  MemoryChunksV1_2_0 = (EFI_REDFISH_MEMORYCHUNKS_V1_2_0 *)InterpProp;
  DestroyMemoryChunks_V1_2_0_CS (MemoryChunksV1_2_0->MemoryChunks); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from MemoryChunksFromEfiStruct
...
)
{
  ...
  return DestroyMemoryChunks_V1_2_0_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishMemoryChunks_V1_2_0EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishMemoryChunks_V1_2_0Unload (
  IN EFI_HANDLE ImageHandle
  )
{
