...
//
#include <RedfishInterpreter/EndpointCollection/EfiEndpointCollection.h>
CHAR8 mResourceTypeStr [] = "#EndpointCollection";
BOOLEAN IsRevisonController = FALSE;

// Support EndpointCollection  
...
{
  EFI_REDFISH_ENDPOINTCOLLECTION *EndpointCollection;
  EFI_REDFISH_ENDPOINTCOLLECTION_CS *EndpointCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "EndpointCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_EndpointCollection_To_CS (ResoruceRaw, &EndpointCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  EndpointCollection = (EFI_REDFISH_ENDPOINTCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_ENDPOINTCOLLECTION));
  if (EndpointCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)EndpointCollection;
  EndpointCollection->EndpointCollection = EndpointCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_ENDPOINTCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("EndpointCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "EndpointCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("EndpointCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "EndpointCollection");
...
EFI_STATUS
EFIAPI
EndpointCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_EndpointCollection_JSON (*((EFI_REDFISH_ENDPOINTCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in EndpointCollectionToStruct.
...
EFI_STATUS
EFIAPI
EndpointCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  EndpointCollection = (EFI_REDFISH_ENDPOINTCOLLECTION *)InterpProp;
  DestroyEndpointCollection_CS (EndpointCollection->EndpointCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from EndpointCollectionFromEfiStruct
...
)
{
  ...
  return DestroyEndpointCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishEndpointCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishEndpointCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
