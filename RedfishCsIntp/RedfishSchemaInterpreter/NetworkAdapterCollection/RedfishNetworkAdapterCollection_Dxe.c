...
//
#include <RedfishInterpreter/NetworkAdapterCollection/EfiNetworkAdapterCollection.h>
CHAR8 mResourceTypeStr [] = "#NetworkAdapterCollection";
...
EFI_STATUS
EFIAPI
NetworkAdapterCollectionToEfiStruct (
...
{
  EFI_REDFISH_NETWORKADAPTERCOLLECTION *NetworkAdapterCollection;
  EFI_REDFISH_NETWORKADAPTERCOLLECTION_CS *NetworkAdapterCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "NetworkAdapterCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_NetworkAdapterCollection_To_CS (ResoruceRaw, &NetworkAdapterCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  NetworkAdapterCollection = (EFI_REDFISH_NETWORKADAPTERCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_NETWORKADAPTERCOLLECTION));
  if (NetworkAdapterCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)NetworkAdapterCollection;
  NetworkAdapterCollection->NetworkAdapterCollection = NetworkAdapterCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_NETWORKADAPTERCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("NetworkAdapterCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "NetworkAdapterCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("NetworkAdapterCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "NetworkAdapterCollection");
...
EFI_STATUS
EFIAPI
NetworkAdapterCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_NetworkAdapterCollection_JSON (*((EFI_REDFISH_NETWORKADAPTERCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in NetworkAdapterCollectionToStruct.
...
EFI_STATUS
EFIAPI
NetworkAdapterCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  NetworkAdapterCollection = (EFI_REDFISH_NETWORKADAPTERCOLLECTION *)InterpProp;
  DestroyNetworkAdapterCollection_CS (NetworkAdapterCollection->NetworkAdapterCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from NetworkAdapterCollectionFromEfiStruct
...
EFI_STATUS
EFIAPI
NetworkAdapterCollectionDestoryJson(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
  IN EFI_REST_RESOURCE_TYPE_IDENTIFIER *RsrcIdentifier,  
  IN CHAR8 *ResoruceRaw
)
{
  ...
  return DestroyNetworkAdapterCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishNetworkAdapterCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishNetworkAdapterCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
