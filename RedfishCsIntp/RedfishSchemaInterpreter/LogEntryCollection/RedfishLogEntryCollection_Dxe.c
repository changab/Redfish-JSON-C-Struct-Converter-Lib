...
//
#include <RedfishInterpreter/LogEntryCollection/EfiLogEntryCollection.h>
CHAR8 mResourceTypeStr [] = "#LogEntryCollection";
BOOLEAN IsRevisonController = FALSE;

// Support LogEntryCollection  
...
{
  EFI_REDFISH_LOGENTRYCOLLECTION *LogEntryCollection;
  EFI_REDFISH_LOGENTRYCOLLECTION_CS *LogEntryCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "LogEntryCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_LogEntryCollection_To_CS (ResoruceRaw, &LogEntryCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  LogEntryCollection = (EFI_REDFISH_LOGENTRYCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_LOGENTRYCOLLECTION));
  if (LogEntryCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)LogEntryCollection;
  LogEntryCollection->LogEntryCollection = LogEntryCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_LOGENTRYCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("LogEntryCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "LogEntryCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("LogEntryCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "LogEntryCollection");
...
EFI_STATUS
EFIAPI
LogEntryCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_LogEntryCollection_JSON (*((EFI_REDFISH_LOGENTRYCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in LogEntryCollectionToStruct.
...
EFI_STATUS
EFIAPI
LogEntryCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  LogEntryCollection = (EFI_REDFISH_LOGENTRYCOLLECTION *)InterpProp;
  DestroyLogEntryCollection_CS (LogEntryCollection->LogEntryCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from LogEntryCollectionFromEfiStruct
...
)
{
  ...
  return DestroyLogEntryCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishLogEntryCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishLogEntryCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
