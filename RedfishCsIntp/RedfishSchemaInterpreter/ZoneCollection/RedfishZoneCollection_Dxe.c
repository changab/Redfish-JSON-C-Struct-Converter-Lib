...
//
#include <RedfishInterpreter/ZoneCollection/EfiZoneCollection.h>
CHAR8 mResourceTypeStr [] = "#ZoneCollection";
BOOLEAN IsRevisonController = FALSE;

// Support ZoneCollection  
...
{
  EFI_REDFISH_ZONECOLLECTION *ZoneCollection;
  EFI_REDFISH_ZONECOLLECTION_CS *ZoneCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "ZoneCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_ZoneCollection_To_CS (ResoruceRaw, &ZoneCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ZoneCollection = (EFI_REDFISH_ZONECOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_ZONECOLLECTION));
  if (ZoneCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)ZoneCollection;
  ZoneCollection->ZoneCollection = ZoneCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_ZONECOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("ZoneCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "ZoneCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("ZoneCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "ZoneCollection");
...
)
{
  ...
  return CS_To_ZoneCollection_JSON (*((EFI_REDFISH_ZONECOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in ZoneCollectionToStruct.
...
EFI_STATUS
EFIAPI
ZoneCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  ZoneCollection = (EFI_REDFISH_ZONECOLLECTION *)InterpProp;
  DestroyZoneCollection_CS (ZoneCollection->ZoneCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from ZoneCollectionFromEfiStruct
...
)
{
  ...
  return DestroyZoneCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishZoneCollectionEntryPoint (
