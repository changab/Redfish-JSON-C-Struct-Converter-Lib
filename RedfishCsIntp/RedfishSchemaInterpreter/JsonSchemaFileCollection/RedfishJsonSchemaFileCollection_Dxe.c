...
//
#include <RedfishInterpreter/JsonSchemaFileCollection/EfiJsonSchemaFileCollection.h>
CHAR8 mResourceTypeStr [] = "#JsonSchemaFileCollection";
BOOLEAN IsRevisonController = FALSE;

// Support JsonSchemaFileCollection  
...
EFI_STATUS
EFIAPI
JsonSchemaFileCollectionToEfiStruct (
...
{
  EFI_REDFISH_JSONSCHEMAFILECOLLECTION *JsonSchemaFileCollection;
  EFI_REDFISH_JSONSCHEMAFILECOLLECTION_CS *JsonSchemaFileCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "JsonSchemaFileCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_JsonSchemaFileCollection_To_CS (ResoruceRaw, &JsonSchemaFileCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  JsonSchemaFileCollection = (EFI_REDFISH_JSONSCHEMAFILECOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_JSONSCHEMAFILECOLLECTION));
  if (JsonSchemaFileCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)JsonSchemaFileCollection;
  JsonSchemaFileCollection->JsonSchemaFileCollection = JsonSchemaFileCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_JSONSCHEMAFILECOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("JsonSchemaFileCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "JsonSchemaFileCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("JsonSchemaFileCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "JsonSchemaFileCollection");
...
EFI_STATUS
EFIAPI
JsonSchemaFileCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_JsonSchemaFileCollection_JSON (*((EFI_REDFISH_JSONSCHEMAFILECOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in JsonSchemaFileCollectionToStruct.
...
EFI_STATUS
EFIAPI
JsonSchemaFileCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  JsonSchemaFileCollection = (EFI_REDFISH_JSONSCHEMAFILECOLLECTION *)InterpProp;
  DestroyJsonSchemaFileCollection_CS (JsonSchemaFileCollection->JsonSchemaFileCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from JsonSchemaFileCollectionFromEfiStruct
...
EFI_STATUS
EFIAPI
JsonSchemaFileCollectionDestoryJson(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
  IN EFI_REST_RESOURCE_TYPE_IDENTIFIER *RsrcIdentifier,  
  IN CHAR8 *ResoruceRaw
)
{
  ...
  return DestroyJsonSchemaFileCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishJsonSchemaFileCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishJsonSchemaFileCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
