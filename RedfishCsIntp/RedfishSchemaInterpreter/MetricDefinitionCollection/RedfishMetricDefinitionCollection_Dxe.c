...
//
#include <RedfishInterpreter/MetricDefinitionCollection/EfiMetricDefinitionCollection.h>
CHAR8 mResourceTypeStr [] = "#MetricDefinitionCollection";
...
EFI_STATUS
EFIAPI
MetricDefinitionCollectionToEfiStruct (
...
{
  EFI_REDFISH_METRICDEFINITIONCOLLECTION *MetricDefinitionCollection;
  EFI_REDFISH_METRICDEFINITIONCOLLECTION_CS *MetricDefinitionCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "MetricDefinitionCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_MetricDefinitionCollection_To_CS (ResoruceRaw, &MetricDefinitionCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  MetricDefinitionCollection = (EFI_REDFISH_METRICDEFINITIONCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_METRICDEFINITIONCOLLECTION));
  if (MetricDefinitionCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)MetricDefinitionCollection;
  MetricDefinitionCollection->MetricDefinitionCollection = MetricDefinitionCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_METRICDEFINITIONCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("MetricDefinitionCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "MetricDefinitionCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("MetricDefinitionCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "MetricDefinitionCollection");
...
EFI_STATUS
EFIAPI
MetricDefinitionCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_MetricDefinitionCollection_JSON (*((EFI_REDFISH_METRICDEFINITIONCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in MetricDefinitionCollectionToStruct.
...
EFI_STATUS
EFIAPI
MetricDefinitionCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  MetricDefinitionCollection = (EFI_REDFISH_METRICDEFINITIONCOLLECTION *)InterpProp;
  DestroyMetricDefinitionCollection_CS (MetricDefinitionCollection->MetricDefinitionCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from MetricDefinitionCollectionFromEfiStruct
...
EFI_STATUS
EFIAPI
MetricDefinitionCollectionDestoryJson(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
  IN EFI_REST_RESOURCE_TYPE_IDENTIFIER *RsrcIdentifier,  
  IN CHAR8 *ResoruceRaw
)
{
  ...
  return DestroyMetricDefinitionCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishMetricDefinitionCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishMetricDefinitionCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
