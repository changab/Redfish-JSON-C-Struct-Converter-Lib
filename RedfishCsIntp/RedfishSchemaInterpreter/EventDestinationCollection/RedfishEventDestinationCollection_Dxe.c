...
//
#include <RedfishInterpreter/EventDestinationCollection/EfiEventDestinationCollection.h>
CHAR8 mResourceTypeStr [] = "#EventDestinationCollection";
...
EFI_STATUS
EFIAPI
EventDestinationCollectionToEfiStruct (
...
{
  EFI_REDFISH_EVENTDESTINATIONCOLLECTION *EventDestinationCollection;
  EFI_REDFISH_EVENTDESTINATIONCOLLECTION_CS *EventDestinationCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "EventDestinationCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_EventDestinationCollection_To_CS (ResoruceRaw, &EventDestinationCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  EventDestinationCollection = (EFI_REDFISH_EVENTDESTINATIONCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_EVENTDESTINATIONCOLLECTION));
  if (EventDestinationCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)EventDestinationCollection;
  EventDestinationCollection->EventDestinationCollection = EventDestinationCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_EVENTDESTINATIONCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("EventDestinationCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "EventDestinationCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("EventDestinationCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "EventDestinationCollection");
...
EFI_STATUS
EFIAPI
EventDestinationCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_EventDestinationCollection_JSON (*((EFI_REDFISH_EVENTDESTINATIONCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in EventDestinationCollectionToStruct.
...
EFI_STATUS
EFIAPI
EventDestinationCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  EventDestinationCollection = (EFI_REDFISH_EVENTDESTINATIONCOLLECTION *)InterpProp;
  DestroyEventDestinationCollection_CS (EventDestinationCollection->EventDestinationCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from EventDestinationCollectionFromEfiStruct
...
EFI_STATUS
EFIAPI
EventDestinationCollectionDestoryJson(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
  IN EFI_REST_RESOURCE_TYPE_IDENTIFIER *RsrcIdentifier,  
  IN CHAR8 *ResoruceRaw
)
{
  ...
  return DestroyEventDestinationCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishEventDestinationCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishEventDestinationCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
