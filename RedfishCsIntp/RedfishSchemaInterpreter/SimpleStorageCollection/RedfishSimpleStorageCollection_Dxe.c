...
//
#include <RedfishInterpreter/SimpleStorageCollection/EfiSimpleStorageCollection.h>
CHAR8 mResourceTypeStr [] = "#SimpleStorageCollection";
BOOLEAN IsRevisonController = FALSE;

// Support SimpleStorageCollection  
...
EFI_STATUS
EFIAPI
SimpleStorageCollectionToEfiStruct (
...
{
  EFI_REDFISH_SIMPLESTORAGECOLLECTION *SimpleStorageCollection;
  EFI_REDFISH_SIMPLESTORAGECOLLECTION_CS *SimpleStorageCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "SimpleStorageCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_SimpleStorageCollection_To_CS (ResoruceRaw, &SimpleStorageCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  SimpleStorageCollection = (EFI_REDFISH_SIMPLESTORAGECOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_SIMPLESTORAGECOLLECTION));
  if (SimpleStorageCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)SimpleStorageCollection;
  SimpleStorageCollection->SimpleStorageCollection = SimpleStorageCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_SIMPLESTORAGECOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("SimpleStorageCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "SimpleStorageCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("SimpleStorageCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "SimpleStorageCollection");
...
EFI_STATUS
EFIAPI
SimpleStorageCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_SimpleStorageCollection_JSON (*((EFI_REDFISH_SIMPLESTORAGECOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in SimpleStorageCollectionToStruct.
...
EFI_STATUS
EFIAPI
SimpleStorageCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  SimpleStorageCollection = (EFI_REDFISH_SIMPLESTORAGECOLLECTION *)InterpProp;
  DestroySimpleStorageCollection_CS (SimpleStorageCollection->SimpleStorageCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from SimpleStorageCollectionFromEfiStruct
...
EFI_STATUS
EFIAPI
SimpleStorageCollectionDestoryJson(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
  IN EFI_REST_RESOURCE_TYPE_IDENTIFIER *RsrcIdentifier,  
  IN CHAR8 *ResoruceRaw
)
{
  ...
  return DestroySimpleStorageCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishSimpleStorageCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishSimpleStorageCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
