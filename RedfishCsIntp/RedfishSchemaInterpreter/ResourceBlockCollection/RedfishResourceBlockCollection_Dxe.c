...
//
#include <RedfishInterpreter/ResourceBlockCollection/EfiResourceBlockCollection.h>
CHAR8 mResourceTypeStr [] = "#ResourceBlockCollection";
BOOLEAN IsRevisonController = FALSE;

// Support ResourceBlockCollection  
...
EFI_STATUS
EFIAPI
ResourceBlockCollectionToEfiStruct (
...
{
  EFI_REDFISH_RESOURCEBLOCKCOLLECTION *ResourceBlockCollection;
  EFI_REDFISH_RESOURCEBLOCKCOLLECTION_CS *ResourceBlockCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "ResourceBlockCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_ResourceBlockCollection_To_CS (ResoruceRaw, &ResourceBlockCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ResourceBlockCollection = (EFI_REDFISH_RESOURCEBLOCKCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_RESOURCEBLOCKCOLLECTION));
  if (ResourceBlockCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)ResourceBlockCollection;
  ResourceBlockCollection->ResourceBlockCollection = ResourceBlockCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_RESOURCEBLOCKCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("ResourceBlockCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "ResourceBlockCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("ResourceBlockCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "ResourceBlockCollection");
...
EFI_STATUS
EFIAPI
ResourceBlockCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_ResourceBlockCollection_JSON (*((EFI_REDFISH_RESOURCEBLOCKCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in ResourceBlockCollectionToStruct.
...
EFI_STATUS
EFIAPI
ResourceBlockCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  ResourceBlockCollection = (EFI_REDFISH_RESOURCEBLOCKCOLLECTION *)InterpProp;
  DestroyResourceBlockCollection_CS (ResourceBlockCollection->ResourceBlockCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from ResourceBlockCollectionFromEfiStruct
...
EFI_STATUS
EFIAPI
ResourceBlockCollectionDestoryJson(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
  IN EFI_REST_RESOURCE_TYPE_IDENTIFIER *RsrcIdentifier,  
  IN CHAR8 *ResoruceRaw
)
{
  ...
  return DestroyResourceBlockCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishResourceBlockCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishResourceBlockCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
