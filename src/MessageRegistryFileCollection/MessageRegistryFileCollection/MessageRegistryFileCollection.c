...
#include"Redfish_MessageRegistryFileCollection_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
//
static RedfishCS_status GenOemCs(RedfishMessageRegistryFileCollection_MessageRegistryFileCollection_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst)
{
...
//
// C structure to JSON for MessageRegistryFileCollection.MessageRegistryFileCollection
//
RedfishCS_status CS_To_MessageRegistryFileCollection_JSON(RedfishMessageRegistryFileCollection_MessageRegistryFileCollection_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
...
//
// Destory C Structure for MessageRegistryFileCollection.MessageRegistryFileCollection
//
RedfishCS_status DestroyMessageRegistryFileCollection_CS(RedfishMessageRegistryFileCollection_MessageRegistryFileCollection_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for MessageRegistryFileCollection.MessageRegistryFileCollection
//
RedfishCS_status DestroyMessageRegistryFileCollection_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for MessageRegistryFileCollection.NOVERSIONED.MessageRegistryFileCollection
//
RedfishCS_status
Json_MessageRegistryFileCollection_To_CS(RedfishCS_char *JsonRawText, RedfishMessageRegistryFileCollection_MessageRegistryFileCollection_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishMessageRegistryFileCollection_MessageRegistryFileCollection_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "MessageRegistryFileCollection", "noversioned", "MessageRegistryFileCollection", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishMessageRegistryFileCollection_MessageRegistryFileCollection_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
  ...
  Status = CreateCsUriOrJsonByNodeArray (Cs, JsonObj, "Members", Cs->Header.ThisUri, &Cs->Members);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){goto Error;}}

  // Members@odata.count
  Status = GetRedfishPropertyInt64 (Cs, JsonObj, "Members@odata.count", &Cs->Membersodata_count);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // Members@odata.nextLink
  Status = GetRedfishPropertyStr (Cs, JsonObj, "Members@odata.nextLink", &Cs->Membersodata_nextLink);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
...
Error:;
  json_decref(JsonObj);
  DestroyMessageRegistryFileCollection_CS (Cs);
  return Status;
}
