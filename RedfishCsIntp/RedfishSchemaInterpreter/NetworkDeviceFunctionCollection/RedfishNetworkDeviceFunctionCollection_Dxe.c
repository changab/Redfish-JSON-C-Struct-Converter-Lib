...
//
#include <RedfishInterpreter/NetworkDeviceFunctionCollection/EfiNetworkDeviceFunctionCollection.h>
CHAR8 mResourceTypeStr [] = "#NetworkDeviceFunctionCollection";
BOOLEAN IsRevisonController = FALSE;

// Support NetworkDeviceFunctionCollection  
...
EFI_STATUS
EFIAPI
NetworkDeviceFunctionCollectionToEfiStruct (
...
{
  EFI_REDFISH_NETWORKDEVICEFUNCTIONCOLLECTION *NetworkDeviceFunctionCollection;
  EFI_REDFISH_NETWORKDEVICEFUNCTIONCOLLECTION_CS *NetworkDeviceFunctionCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "NetworkDeviceFunctionCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_NetworkDeviceFunctionCollection_To_CS (ResoruceRaw, &NetworkDeviceFunctionCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  NetworkDeviceFunctionCollection = (EFI_REDFISH_NETWORKDEVICEFUNCTIONCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_NETWORKDEVICEFUNCTIONCOLLECTION));
  if (NetworkDeviceFunctionCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)NetworkDeviceFunctionCollection;
  NetworkDeviceFunctionCollection->NetworkDeviceFunctionCollection = NetworkDeviceFunctionCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_NETWORKDEVICEFUNCTIONCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("NetworkDeviceFunctionCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "NetworkDeviceFunctionCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("NetworkDeviceFunctionCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "NetworkDeviceFunctionCollection");
...
EFI_STATUS
EFIAPI
NetworkDeviceFunctionCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_NetworkDeviceFunctionCollection_JSON (*((EFI_REDFISH_NETWORKDEVICEFUNCTIONCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in NetworkDeviceFunctionCollectionToStruct.
...
EFI_STATUS
EFIAPI
NetworkDeviceFunctionCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  NetworkDeviceFunctionCollection = (EFI_REDFISH_NETWORKDEVICEFUNCTIONCOLLECTION *)InterpProp;
  DestroyNetworkDeviceFunctionCollection_CS (NetworkDeviceFunctionCollection->NetworkDeviceFunctionCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from NetworkDeviceFunctionCollectionFromEfiStruct
...
EFI_STATUS
EFIAPI
NetworkDeviceFunctionCollectionDestoryJson(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
  IN EFI_REST_RESOURCE_TYPE_IDENTIFIER *RsrcIdentifier,  
  IN CHAR8 *ResoruceRaw
)
{
  ...
  return DestroyNetworkDeviceFunctionCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishNetworkDeviceFunctionCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishNetworkDeviceFunctionCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
