...
#include"Redfish_EventDestinationCollection_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
//
static RedfishCS_status GenOemCs(RedfishEventDestinationCollection_EventDestinationCollection_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst)
{
...
//
// C structure to JSON for EventDestinationCollection.EventDestinationCollection
//
RedfishCS_status CS_To_EventDestinationCollection_JSON(RedfishEventDestinationCollection_EventDestinationCollection_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
...
//
// Destory C Structure for EventDestinationCollection.EventDestinationCollection
//
RedfishCS_status DestroyEventDestinationCollection_CS(RedfishEventDestinationCollection_EventDestinationCollection_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for EventDestinationCollection.EventDestinationCollection
//
RedfishCS_status DestroyEventDestinationCollection_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for EventDestinationCollection.NOVERSIONED.EventDestinationCollection
//
RedfishCS_status
Json_EventDestinationCollection_To_CS(RedfishCS_char *JsonRawText, RedfishEventDestinationCollection_EventDestinationCollection_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishEventDestinationCollection_EventDestinationCollection_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "EventDestinationCollection", "noversioned", "EventDestinationCollection", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishEventDestinationCollection_EventDestinationCollection_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
...
Error:;
  json_decref(JsonObj);
  DestroyEventDestinationCollection_CS (Cs);
  return Status;
}
