...
//
#include <RedfishInterpreter/ManagerCollection/EfiManagerCollection.h>
CHAR8 mResourceTypeStr [] = "#ManagerCollection";
BOOLEAN IsRevisonController = FALSE;

// Support ManagerCollection  
...
{
  EFI_REDFISH_MANAGERCOLLECTION *ManagerCollection;
  EFI_REDFISH_MANAGERCOLLECTION_CS *ManagerCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "ManagerCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_ManagerCollection_To_CS (ResoruceRaw, &ManagerCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ManagerCollection = (EFI_REDFISH_MANAGERCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_MANAGERCOLLECTION));
  if (ManagerCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)ManagerCollection;
  ManagerCollection->ManagerCollection = ManagerCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_MANAGERCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("ManagerCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "ManagerCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("ManagerCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "ManagerCollection");
...
EFI_STATUS
EFIAPI
ManagerCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_ManagerCollection_JSON (*((EFI_REDFISH_MANAGERCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in ManagerCollectionToStruct.
...
EFI_STATUS
EFIAPI
ManagerCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  ManagerCollection = (EFI_REDFISH_MANAGERCOLLECTION *)InterpProp;
  DestroyManagerCollection_CS (ManagerCollection->ManagerCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from ManagerCollectionFromEfiStruct
...
)
{
  ...
  return DestroyManagerCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishManagerCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishManagerCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
