...
#include"Redfish_JobCollection_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
//
static RedfishCS_status GenOemCs(RedfishJobCollection_JobCollection_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst)
{
...
//
// C structure to JSON for JobCollection.JobCollection
//
RedfishCS_status CS_To_JobCollection_JSON(RedfishJobCollection_JobCollection_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
...
//
// Destory C Structure for JobCollection.JobCollection
//
RedfishCS_status DestroyJobCollection_CS(RedfishJobCollection_JobCollection_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for JobCollection.JobCollection
//
RedfishCS_status DestroyJobCollection_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for JobCollection.NOVERSIONED.JobCollection
//
RedfishCS_status
Json_JobCollection_To_CS(RedfishCS_char *JsonRawText, RedfishJobCollection_JobCollection_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishJobCollection_JobCollection_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "JobCollection", "noversioned", "JobCollection", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishJobCollection_JobCollection_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
