...
//
#include <RedfishInterpreter/SerialInterfaceCollection/EfiSerialInterfaceCollection.h>
CHAR8 mResourceTypeStr [] = "#SerialInterfaceCollection";
BOOLEAN IsRevisonController = FALSE;

// Support SerialInterfaceCollection  
...
EFI_STATUS
EFIAPI
SerialInterfaceCollectionToEfiStruct (
...
{
  EFI_REDFISH_SERIALINTERFACECOLLECTION *SerialInterfaceCollection;
  EFI_REDFISH_SERIALINTERFACECOLLECTION_CS *SerialInterfaceCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "SerialInterfaceCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_SerialInterfaceCollection_To_CS (ResoruceRaw, &SerialInterfaceCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  SerialInterfaceCollection = (EFI_REDFISH_SERIALINTERFACECOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_SERIALINTERFACECOLLECTION));
  if (SerialInterfaceCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)SerialInterfaceCollection;
  SerialInterfaceCollection->SerialInterfaceCollection = SerialInterfaceCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_SERIALINTERFACECOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("SerialInterfaceCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "SerialInterfaceCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("SerialInterfaceCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "SerialInterfaceCollection");
...
EFI_STATUS
EFIAPI
SerialInterfaceCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_SerialInterfaceCollection_JSON (*((EFI_REDFISH_SERIALINTERFACECOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in SerialInterfaceCollectionToStruct.
...
EFI_STATUS
EFIAPI
SerialInterfaceCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  SerialInterfaceCollection = (EFI_REDFISH_SERIALINTERFACECOLLECTION *)InterpProp;
  DestroySerialInterfaceCollection_CS (SerialInterfaceCollection->SerialInterfaceCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from SerialInterfaceCollectionFromEfiStruct
...
EFI_STATUS
EFIAPI
SerialInterfaceCollectionDestoryJson(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
  IN EFI_REST_RESOURCE_TYPE_IDENTIFIER *RsrcIdentifier,  
  IN CHAR8 *ResoruceRaw
)
{
  ...
  return DestroySerialInterfaceCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishSerialInterfaceCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishSerialInterfaceCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
