...
//
#include <RedfishInterpreter/RoleCollection/EfiRoleCollection.h>
CHAR8 mResourceTypeStr [] = "#RoleCollection";
BOOLEAN IsRevisonController = FALSE;

// Support RoleCollection  
...
{
  EFI_REDFISH_ROLECOLLECTION *RoleCollection;
  EFI_REDFISH_ROLECOLLECTION_CS *RoleCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "RoleCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_RoleCollection_To_CS (ResoruceRaw, &RoleCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  RoleCollection = (EFI_REDFISH_ROLECOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_ROLECOLLECTION));
  if (RoleCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)RoleCollection;
  RoleCollection->RoleCollection = RoleCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_ROLECOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("RoleCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "RoleCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("RoleCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "RoleCollection");
...
)
{
  ...
  return CS_To_RoleCollection_JSON (*((EFI_REDFISH_ROLECOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in RoleCollectionToStruct.
...
EFI_STATUS
EFIAPI
RoleCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  RoleCollection = (EFI_REDFISH_ROLECOLLECTION *)InterpProp;
  DestroyRoleCollection_CS (RoleCollection->RoleCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from RoleCollectionFromEfiStruct
...
)
{
  ...
  return DestroyRoleCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishRoleCollectionEntryPoint (
