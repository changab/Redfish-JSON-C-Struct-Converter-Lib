...
//
#include <RedfishInterpreter/NetworkPort/v1_2_1/EfiNetworkPortV1_2_1.h>
CHAR8 mResourceTypeStr [] = "#NetworkPort";
BOOLEAN IsRevisonController = TRUE;

// Support NetworkPort V1_2_1 
...
{
  EFI_REDFISH_NETWORKPORT_V1_2_1 *NetworkPortV1_2_1;
  EFI_REDFISH_NETWORKPORT_V1_2_1_CS *NetworkPortV1_2_1Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "NetworkPort") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_NetworkPort_V1_2_1_To_CS (ResoruceRaw, &NetworkPortV1_2_1Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  NetworkPortV1_2_1 = (EFI_REDFISH_NETWORKPORT_V1_2_1 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_NETWORKPORT_V1_2_1));
  if (NetworkPortV1_2_1 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)NetworkPortV1_2_1;
  NetworkPortV1_2_1->NetworkPort = NetworkPortV1_2_1Cs;
  ResourceTypeMajorVersion = "1";
  ResourceTypeMinorVersion = "2";
  ResourceTypeErrataVersion = "1";
  Length = sizeof (EFI_REDFISH_NETWORKPORT_V1_2_1_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("NetworkPort"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "NetworkPort");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("NetworkPort"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "NetworkPort");
...
)
{
  ...
  return CS_To_NetworkPort_V1_2_1_JSON (*((EFI_REDFISH_NETWORKPORT_V1_2_1_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in NetworkPortToStruct.
...
)
{
  ...
  NetworkPortV1_2_1 = (EFI_REDFISH_NETWORKPORT_V1_2_1 *)InterpProp;
  DestroyNetworkPort_V1_2_1_CS (NetworkPortV1_2_1->NetworkPort); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from NetworkPortFromEfiStruct
...
)
{
  ...
  return DestroyNetworkPort_V1_2_1_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishNetworkPort_V1_2_1EntryPoint (
...
EFI_STATUS
EFIAPI
RedfishNetworkPort_V1_2_1Unload (
  IN EFI_HANDLE ImageHandle
  )
{
