...
//
#include <RedfishInterpreter/TriggersCollection/EfiTriggersCollection.h>
CHAR8 mResourceTypeStr [] = "#TriggersCollection";
BOOLEAN IsRevisonController = FALSE;

// Support TriggersCollection  
...
{
  EFI_REDFISH_TRIGGERSCOLLECTION *TriggersCollection;
  EFI_REDFISH_TRIGGERSCOLLECTION_CS *TriggersCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "TriggersCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_TriggersCollection_To_CS (ResoruceRaw, &TriggersCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  TriggersCollection = (EFI_REDFISH_TRIGGERSCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_TRIGGERSCOLLECTION));
  if (TriggersCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)TriggersCollection;
  TriggersCollection->TriggersCollection = TriggersCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_TRIGGERSCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("TriggersCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "TriggersCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("TriggersCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "TriggersCollection");
...
EFI_STATUS
EFIAPI
TriggersCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_TriggersCollection_JSON (*((EFI_REDFISH_TRIGGERSCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in TriggersCollectionToStruct.
...
EFI_STATUS
EFIAPI
TriggersCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  TriggersCollection = (EFI_REDFISH_TRIGGERSCOLLECTION *)InterpProp;
  DestroyTriggersCollection_CS (TriggersCollection->TriggersCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from TriggersCollectionFromEfiStruct
...
)
{
  ...
  return DestroyTriggersCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishTriggersCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishTriggersCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
