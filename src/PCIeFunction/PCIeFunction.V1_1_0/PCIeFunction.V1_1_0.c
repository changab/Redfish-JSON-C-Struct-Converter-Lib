...
#include"Redfish_PCIeFunction_v1_1_0_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
static RedfishCS_status GenOemCs(RedfishPCIeFunction_V1_1_0_PCIeFunction_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst);
//
//Generate C structure for Oem
//
static RedfishCS_status GenActions_OemCs(RedfishPCIeFunction_V1_1_0_PCIeFunction_CS *Cs, json_t *JsonObj, char *Key, RedfishPCIeFunction_V1_1_0_PCIeFunction_Actions_Oem_CS **Dst)
{
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishPCIeFunction_V1_1_0_PCIeFunction_Actions_Oem_CS), (RedfishCS_void **)Dst);
...
//
static RedfishCS_status GenActionsCs(RedfishPCIeFunction_V1_1_0_PCIeFunction_CS *Cs, json_t *JsonObj, char *Key, RedfishPCIeFunction_V1_1_0_PCIeFunction_Actions_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishPCIeFunction_V1_1_0_PCIeFunction_Actions_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // Oem
...
//
static RedfishCS_status GenLinksCs(RedfishPCIeFunction_V1_1_0_PCIeFunction_CS *Cs, json_t *JsonObj, char *Key, RedfishPCIeFunction_V1_1_0_PCIeFunction_Links_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishPCIeFunction_V1_1_0_PCIeFunction_Links_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // Drives
  ...
  // EthernetInterfaces
  InitializeLinkHead (&(*Dst)->EthernetInterfaces);
  Status = CreateCsUriOrJsonByNodeArray (Cs, TempJsonObj, "EthernetInterfaces", Cs->Header.ThisUri, &(*Dst)->EthernetInterfaces);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // EthernetInterfaces@odata.count
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "EthernetInterfaces@odata.count", &(*Dst)->EthernetInterfacesodata_count);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // EthernetInterfaces@odata.navigationLink
  Status = GetRedfishPropertyStr (Cs, TempJsonObj, "EthernetInterfaces@odata.navigationLink", &(*Dst)->EthernetInterfacesodata_navigationLink);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
  ...
  // StorageControllers
  InitializeLinkHead (&(*Dst)->StorageControllers);
  Status = CreateCsUriOrJsonByNodeArray (Cs, TempJsonObj, "StorageControllers", Cs->Header.ThisUri, &(*Dst)->StorageControllers);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // StorageControllers@odata.count
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "StorageControllers@odata.count", &(*Dst)->StorageControllersodata_count);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // StorageControllers@odata.navigationLink
  Status = GetRedfishPropertyStr (Cs, TempJsonObj, "StorageControllers@odata.navigationLink", &(*Dst)->StorageControllersodata_navigationLink);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
...
//
static RedfishCS_status GenStatusCs(RedfishPCIeFunction_V1_1_0_PCIeFunction_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Status_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
...
static RedfishCS_status CS_To_JSON_ActionsOem(json_t *CsJson, char *Key, RedfishPCIeFunction_V1_1_0_PCIeFunction_Actions_Oem_CS *CSPtr)
{
  if (CSPtr == NULL) {
...
static RedfishCS_status CS_To_JSON_Actions(json_t *CsJson, char *Key, RedfishPCIeFunction_V1_1_0_PCIeFunction_Actions_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
static RedfishCS_status CS_To_JSON_Links(json_t *CsJson, char *Key, RedfishPCIeFunction_V1_1_0_PCIeFunction_Links_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
//
// C structure to JSON for PCIeFunction.V1_1_0.PCIeFunction
//
RedfishCS_status CS_To_PCIeFunction_V1_1_0_JSON(RedfishPCIeFunction_V1_1_0_PCIeFunction_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
  ...
  // DeviceClass 
  if (InsertJsonStringObj (CsJson, "DeviceClass", CSPtr->DeviceClass) != RedfishCS_status_success) {goto Error;}

  // DeviceId 
  if (InsertJsonStringObj (CsJson, "DeviceId", CSPtr->DeviceId) != RedfishCS_status_success) {goto Error;}

  // FunctionId 
  if (InsertJsonInt64Obj (CsJson, "FunctionId", CSPtr->FunctionId) != RedfishCS_status_success) {goto Error;}

  // FunctionType 
  if (InsertJsonStringObj (CsJson, "FunctionType", CSPtr->FunctionType) != RedfishCS_status_success) {goto Error;}
  ...
  // RevisionId 
  if (InsertJsonStringObj (CsJson, "RevisionId", CSPtr->RevisionId) != RedfishCS_status_success) {goto Error;}
...
//
// Destory C Structure for PCIeFunction.V1_1_0.PCIeFunction
//
RedfishCS_status DestroyPCIeFunction_V1_1_0_CS(RedfishPCIeFunction_V1_1_0_PCIeFunction_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for PCIeFunction.V1_1_0.PCIeFunction
//
RedfishCS_status DestroyPCIeFunction_V1_1_0_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for PCIeFunction.V1_1_0.PCIeFunction
//
RedfishCS_status
Json_PCIeFunction_V1_1_0_To_CS(RedfishCS_char *JsonRawText, RedfishPCIeFunction_V1_1_0_PCIeFunction_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishPCIeFunction_V1_1_0_PCIeFunction_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "PCIeFunction", "v1_1_0", "PCIeFunction", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishPCIeFunction_V1_1_0_PCIeFunction_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
  ...
  // ClassCode
  Status = GetRedfishPropertyStr (Cs, JsonObj, "ClassCode", &Cs->ClassCode);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
  ...
  // DeviceClass
  Status = GetRedfishPropertyStr (Cs, JsonObj, "DeviceClass", &Cs->DeviceClass);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
  ...
  // FunctionId
  Status = GetRedfishPropertyInt64 (Cs, JsonObj, "FunctionId", &Cs->FunctionId);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // FunctionType
  Status = GetRedfishPropertyStr (Cs, JsonObj, "FunctionType", &Cs->FunctionType);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
  ...
  // RevisionId
  Status = GetRedfishPropertyStr (Cs, JsonObj, "RevisionId", &Cs->RevisionId);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
  ...
  // SubsystemId
  Status = GetRedfishPropertyStr (Cs, JsonObj, "SubsystemId", &Cs->SubsystemId);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
...
Error:;
  json_decref(JsonObj);
  DestroyPCIeFunction_V1_1_0_CS (Cs);
  return Status;
}
