...
//
#include <RedfishInterpreter/HostInterfaceCollection/EfiHostInterfaceCollection.h>
CHAR8 mResourceTypeStr [] = "#HostInterfaceCollection";
BOOLEAN IsRevisonController = FALSE;

// Support HostInterfaceCollection  
...
EFI_STATUS
EFIAPI
HostInterfaceCollectionToEfiStruct (
...
{
  EFI_REDFISH_HOSTINTERFACECOLLECTION *HostInterfaceCollection;
  EFI_REDFISH_HOSTINTERFACECOLLECTION_CS *HostInterfaceCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "HostInterfaceCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_HostInterfaceCollection_To_CS (ResoruceRaw, &HostInterfaceCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  HostInterfaceCollection = (EFI_REDFISH_HOSTINTERFACECOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_HOSTINTERFACECOLLECTION));
  if (HostInterfaceCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)HostInterfaceCollection;
  HostInterfaceCollection->HostInterfaceCollection = HostInterfaceCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_HOSTINTERFACECOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("HostInterfaceCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "HostInterfaceCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("HostInterfaceCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "HostInterfaceCollection");
...
EFI_STATUS
EFIAPI
HostInterfaceCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_HostInterfaceCollection_JSON (*((EFI_REDFISH_HOSTINTERFACECOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in HostInterfaceCollectionToStruct.
...
EFI_STATUS
EFIAPI
HostInterfaceCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  HostInterfaceCollection = (EFI_REDFISH_HOSTINTERFACECOLLECTION *)InterpProp;
  DestroyHostInterfaceCollection_CS (HostInterfaceCollection->HostInterfaceCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from HostInterfaceCollectionFromEfiStruct
...
EFI_STATUS
EFIAPI
HostInterfaceCollectionDestoryJson(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
  IN EFI_REST_RESOURCE_TYPE_IDENTIFIER *RsrcIdentifier,  
  IN CHAR8 *ResoruceRaw
)
{
  ...
  return DestroyHostInterfaceCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishHostInterfaceCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishHostInterfaceCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
