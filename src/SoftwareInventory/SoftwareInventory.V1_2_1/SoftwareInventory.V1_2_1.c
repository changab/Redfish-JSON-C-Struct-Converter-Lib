...
#include"Redfish_SoftwareInventory_v1_2_1_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
static RedfishCS_status GenOemCs(RedfishSoftwareInventory_V1_2_1_SoftwareInventory_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst);
//
//Generate C structure for Oem
//
static RedfishCS_status GenOemActionsCs(RedfishSoftwareInventory_V1_2_1_SoftwareInventory_CS *Cs, json_t *JsonObj, char *Key, RedfishSoftwareInventory_V1_2_1_OemActions_CS **Dst)
{
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishSoftwareInventory_V1_2_1_OemActions_CS), (RedfishCS_void **)Dst);
...
//
static RedfishCS_status GenActionsCs(RedfishSoftwareInventory_V1_2_1_SoftwareInventory_CS *Cs, json_t *JsonObj, char *Key, RedfishSoftwareInventory_V1_2_1_Actions_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishSoftwareInventory_V1_2_1_Actions_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // Oem
...
static RedfishCS_status GenidRef_Array_ElementCs(RedfishSoftwareInventory_V1_2_1_SoftwareInventory_CS *Cs, json_t *JsonObj, RedfishCS_uint64 Index,  Redfishodata_V4_0_3_idRef_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
...
//
static RedfishCS_status GenidRef_ArrayCs(RedfishSoftwareInventory_V1_2_1_SoftwareInventory_CS *Cs, json_t *JsonObj, char *Key, Redfishodata_V4_0_3_idRef_Array_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
...
//
static RedfishCS_status GenStatusCs(RedfishSoftwareInventory_V1_2_1_SoftwareInventory_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Status_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
...
//
static RedfishCS_status GenRedfishCS_char_ArrayCs(RedfishSoftwareInventory_V1_2_1_SoftwareInventory_CS *Cs, json_t *JsonObj, char *Key, RedfishCS_char_Array **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  RedfishCS_uint64 ArraySize;
...
static RedfishCS_status CS_To_JSON_ActionsOem(json_t *CsJson, char *Key, RedfishSoftwareInventory_V1_2_1_OemActions_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
static RedfishCS_status CS_To_JSON_Actions(json_t *CsJson, char *Key, RedfishSoftwareInventory_V1_2_1_Actions_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
static RedfishCS_status CS_To_JSON_RelatedItem(json_t *CsJson, char *Key, Redfishodata_V4_0_3_idRef_Array_CS *CSPtr)
{
  json_t *ArrayJson;
  json_t *ArrayMember;
...
//
// C structure to JSON for SoftwareInventory.V1_2_1.SoftwareInventory
//
RedfishCS_status CS_To_SoftwareInventory_V1_2_1_JSON(RedfishSoftwareInventory_V1_2_1_SoftwareInventory_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
  ...
  if (InsertJsonStringObj (CsJson, "LowestSupportedVersion", CSPtr->LowestSupportedVersion) != RedfishCS_status_success) {goto Error;}
  ...
  // RelatedItem
  if (CS_To_JSON_RelatedItem(CsJson, "RelatedItem", CSPtr->RelatedItem) != RedfishCS_status_success) {goto Error;}

  // RelatedItem@odata.count 
  if (InsertJsonInt64Obj (CsJson, "RelatedItem@odata.count", CSPtr->RelatedItemodata_count) != RedfishCS_status_success) {goto Error;}

  // ReleaseDate 
  if (InsertJsonStringObj (CsJson, "ReleaseDate", CSPtr->ReleaseDate) != RedfishCS_status_success) {goto Error;}

  // SoftwareId 
  if (InsertJsonStringObj (CsJson, "SoftwareId", CSPtr->SoftwareId) != RedfishCS_status_success) {goto Error;}

  // Status
  if (CS_To_JSON_Status(CsJson, "Status", CSPtr->Status) != RedfishCS_status_success) {goto Error;}

  // UefiDevicePaths 
  if (InsertJsonStringArrayObj (CsJson, "UefiDevicePaths", CSPtr->UefiDevicePaths) != RedfishCS_status_success) {goto Error;}

  // Updateable 
  if (InsertJsonBoolObj (CsJson, "Updateable", CSPtr->Updateable) != RedfishCS_status_success) {goto Error;}
...
//
// Destory C Structure for SoftwareInventory.V1_2_1.SoftwareInventory
//
RedfishCS_status DestroySoftwareInventory_V1_2_1_CS(RedfishSoftwareInventory_V1_2_1_SoftwareInventory_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for SoftwareInventory.V1_2_1.SoftwareInventory
//
RedfishCS_status DestroySoftwareInventory_V1_2_1_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for SoftwareInventory.V1_2_1.SoftwareInventory
//
RedfishCS_status
Json_SoftwareInventory_V1_2_1_To_CS(RedfishCS_char *JsonRawText, RedfishSoftwareInventory_V1_2_1_SoftwareInventory_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishSoftwareInventory_V1_2_1_SoftwareInventory_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "SoftwareInventory", "v1_2_1", "SoftwareInventory", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishSoftwareInventory_V1_2_1_SoftwareInventory_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
  ...
  // LowestSupportedVersion
  Status = GetRedfishPropertyStr (Cs, JsonObj, "LowestSupportedVersion", &Cs->LowestSupportedVersion);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
  ...
  // RelatedItem@odata.count
  Status = GetRedfishPropertyInt64 (Cs, JsonObj, "RelatedItem@odata.count", &Cs->RelatedItemodata_count);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // ReleaseDate
  Status = GetRedfishPropertyStr (Cs, JsonObj, "ReleaseDate", &Cs->ReleaseDate);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // SoftwareId
  Status = GetRedfishPropertyStr (Cs, JsonObj, "SoftwareId", &Cs->SoftwareId);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
  ...
  // UefiDevicePaths
  Status = GenRedfishCS_char_ArrayCs (Cs, JsonObj, "UefiDevicePaths", &Cs->UefiDevicePaths);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // Updateable
  Status = GetRedfishPropertyBoolean (Cs, JsonObj, "Updateable", &Cs->Updateable);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
...
Error:;
  json_decref(JsonObj);
  DestroySoftwareInventory_V1_2_1_CS (Cs);
  return Status;
}
