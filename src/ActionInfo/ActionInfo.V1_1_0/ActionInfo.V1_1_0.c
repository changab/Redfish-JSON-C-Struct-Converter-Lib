...
#include"Redfish_ActionInfo_v1_1_0_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
//
static RedfishCS_status GenOemCs(RedfishActionInfo_V1_1_0_ActionInfo_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst)
{
...
//
static RedfishCS_status GenRedfishCS_char_ArrayCs(RedfishActionInfo_V1_1_0_ActionInfo_CS *Cs, json_t *JsonObj, char *Key, RedfishCS_char_Array **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
...
static RedfishCS_status GenParameters_Array_ElementCs(RedfishActionInfo_V1_1_0_ActionInfo_CS *Cs, json_t *JsonObj, RedfishCS_uint64 Index,  RedfishActionInfo_V1_1_0_Parameters_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsZeroMemory(Cs, sizeof(RedfishActionInfo_V1_1_0_Parameters_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // AllowableValues
  Status = GenRedfishCS_char_ArrayCs (Cs, TempJsonObj, "AllowableValues", &(*Dst)->AllowableValues);
  ...
  // DataType
  Status = GetRedfishPropertyStr (Cs, TempJsonObj, "DataType", &(*Dst)->DataType);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // MaximumValue
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "MaximumValue", &(*Dst)->MaximumValue);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // MinimumValue
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "MinimumValue", &(*Dst)->MinimumValue);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
  ...
  // ObjectDataType
  Status = GetRedfishPropertyStr (Cs, TempJsonObj, "ObjectDataType", &(*Dst)->ObjectDataType);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // Required
  Status = GetRedfishPropertyBoolean (Cs, TempJsonObj, "Required", &(*Dst)->Required);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
...
//
//Generate C structure for RedfishActionInfo_V1_1_0_Parameters_Array_CS
//
static RedfishCS_status GenParameters_ArrayCs(RedfishActionInfo_V1_1_0_ActionInfo_CS *Cs, json_t *JsonObj, char *Key, RedfishActionInfo_V1_1_0_Parameters_Array_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  RedfishCS_uint64 ArraySize;
  RedfishCS_uint64 Index;
  RedfishActionInfo_V1_1_0_Parameters_Array_CS *ThisElement;
  ...
  ArraySize = json_array_size (TempJsonObj);
  Status = allocateArrayRecordCsMemory(Cs, sizeof (RedfishActionInfo_V1_1_0_Parameters_Array_CS), ArraySize, (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  if (*Dst == NULL) {
  ...
  ThisElement = *Dst;
  for (Index = 0; Index < ArraySize; Index ++) {
    Status = GenParameters_Array_ElementCs(Cs, TempJsonObj, Index, &ThisElement->ArrayValue);
    if (Status != RedfishCS_status_success){
      goto Error;
    }
    ThisElement = ThisElement->Next;
...
static RedfishCS_status CS_To_JSON_Parameters(json_t *CsJson, char *Key, RedfishActionInfo_V1_1_0_Parameters_Array_CS *CSPtr)
{
  json_t *ArrayJson;
  json_t *ArrayMember;
  RedfishActionInfo_V1_1_0_Parameters_Array_CS *NextArray;
  RedfishActionInfo_V1_1_0_Parameters_CS *NextArrayItem;
  ...
  do {
    ...
    // AllowableValues 
    if (InsertJsonStringArrayObj (ArrayMember, "AllowableValues", NextArrayItem->AllowableValues) != RedfishCS_status_success) {goto Error;}

    // DataType 
    if (InsertJsonStringObj (ArrayMember, "DataType", NextArrayItem->DataType) != RedfishCS_status_success) {goto Error;}

    // MaximumValue 
    if (InsertJsonInt64Obj (ArrayMember, "MaximumValue", NextArrayItem->MaximumValue) != RedfishCS_status_success) {goto Error;}

    // MinimumValue 
    if (InsertJsonInt64Obj (ArrayMember, "MinimumValue", NextArrayItem->MinimumValue) != RedfishCS_status_success) {goto Error;}

    // Name 
    if (InsertJsonStringObj (ArrayMember, "Name", NextArrayItem->Name) != RedfishCS_status_success) {goto Error;}

    // ObjectDataType 
    if (InsertJsonStringObj (ArrayMember, "ObjectDataType", NextArrayItem->ObjectDataType) != RedfishCS_status_success) {goto Error;}

    // Required 
    if (InsertJsonBoolObj (ArrayMember, "Required", NextArrayItem->Required) != RedfishCS_status_success) {goto Error;}
...
//
// C structure to JSON for ActionInfo.V1_1_0.ActionInfo
//
RedfishCS_status CS_To_ActionInfo_V1_1_0_JSON(RedfishActionInfo_V1_1_0_ActionInfo_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
  ...
  // Parameters
  if (CS_To_JSON_Parameters(CsJson, "Parameters", CSPtr->Parameters) != RedfishCS_status_success) {goto Error;}
...
//
// Destory C Structure for ActionInfo.V1_1_0.ActionInfo
//
RedfishCS_status DestroyActionInfo_V1_1_0_CS(RedfishActionInfo_V1_1_0_ActionInfo_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for ActionInfo.V1_1_0.ActionInfo
//
RedfishCS_status DestroyActionInfo_V1_1_0_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for ActionInfo.V1_1_0.ActionInfo
//
RedfishCS_status
Json_ActionInfo_V1_1_0_To_CS(RedfishCS_char *JsonRawText, RedfishActionInfo_V1_1_0_ActionInfo_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishActionInfo_V1_1_0_ActionInfo_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "ActionInfo", "v1_1_0", "ActionInfo", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishActionInfo_V1_1_0_ActionInfo_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
  ...
  // Parameters
  Status = GenParameters_ArrayCs (Cs, JsonObj, "Parameters", &Cs->Parameters);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
...
Error:;
  json_decref(JsonObj);
  DestroyActionInfo_V1_1_0_CS (Cs);
  return Status;
}
