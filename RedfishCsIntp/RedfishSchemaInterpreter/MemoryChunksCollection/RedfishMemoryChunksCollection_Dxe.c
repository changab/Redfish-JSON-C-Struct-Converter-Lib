...
//
#include <RedfishInterpreter/MemoryChunksCollection/EfiMemoryChunksCollection.h>
CHAR8 mResourceTypeStr [] = "#MemoryChunksCollection";
BOOLEAN IsRevisonController = FALSE;

// Support MemoryChunksCollection  
...
EFI_STATUS
EFIAPI
MemoryChunksCollectionToEfiStruct (
...
{
  EFI_REDFISH_MEMORYCHUNKSCOLLECTION *MemoryChunksCollection;
  EFI_REDFISH_MEMORYCHUNKSCOLLECTION_CS *MemoryChunksCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "MemoryChunksCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_MemoryChunksCollection_To_CS (ResoruceRaw, &MemoryChunksCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  MemoryChunksCollection = (EFI_REDFISH_MEMORYCHUNKSCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_MEMORYCHUNKSCOLLECTION));
  if (MemoryChunksCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)MemoryChunksCollection;
  ...
  Length = sizeof (EFI_REDFISH_MEMORYCHUNKSCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("MemoryChunksCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "MemoryChunksCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("MemoryChunksCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "MemoryChunksCollection");
...
EFI_STATUS
EFIAPI
MemoryChunksCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_MemoryChunksCollection_JSON (*((EFI_REDFISH_MEMORYCHUNKSCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in MemoryChunksCollectionToStruct.
...
EFI_STATUS
EFIAPI
MemoryChunksCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  MemoryChunksCollection = (EFI_REDFISH_MEMORYCHUNKSCOLLECTION *)InterpProp;
  DestroyMemoryChunksCollection_CS (MemoryChunksCollection->MemoryChunksCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from MemoryChunksCollectionFromEfiStruct
...
EFI_STATUS
EFIAPI
MemoryChunksCollectionDestoryJson(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
  IN EFI_REST_RESOURCE_TYPE_IDENTIFIER *RsrcIdentifier,  
  IN CHAR8 *ResoruceRaw
)
{
  ...
  return DestroyMemoryChunksCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishMemoryChunksCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishMemoryChunksCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
