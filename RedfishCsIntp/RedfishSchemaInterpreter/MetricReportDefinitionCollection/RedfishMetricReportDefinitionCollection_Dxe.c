...
//
#include <RedfishInterpreter/MetricReportDefinitionCollection/EfiMetricReportDefinitionCollection.h>
CHAR8 mResourceTypeStr [] = "#MetricReportDefinitionCollection";
BOOLEAN IsRevisonController = FALSE;

// Support MetricReportDefinitionCollection  
...
EFI_STATUS
EFIAPI
MetricReportDefinitionCollectionToEfiStruct (
...
{
  EFI_REDFISH_METRICREPORTDEFINITIONCOLLECTION *MetricReportDefinitionCollection;
  EFI_REDFISH_METRICREPORTDEFINITIONCOLLECTION_CS *MetricReportDefinitionCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "MetricReportDefinitionCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_MetricReportDefinitionCollection_To_CS (ResoruceRaw, &MetricReportDefinitionCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  MetricReportDefinitionCollection = (EFI_REDFISH_METRICREPORTDEFINITIONCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_METRICREPORTDEFINITIONCOLLECTION));
  if (MetricReportDefinitionCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)MetricReportDefinitionCollection;
  MetricReportDefinitionCollection->MetricReportDefinitionCollection = MetricReportDefinitionCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_METRICREPORTDEFINITIONCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("MetricReportDefinitionCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "MetricReportDefinitionCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("MetricReportDefinitionCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "MetricReportDefinitionCollection");
...
EFI_STATUS
EFIAPI
MetricReportDefinitionCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_MetricReportDefinitionCollection_JSON (*((EFI_REDFISH_METRICREPORTDEFINITIONCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in MetricReportDefinitionCollectionToStruct.
...
EFI_STATUS
EFIAPI
MetricReportDefinitionCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  MetricReportDefinitionCollection = (EFI_REDFISH_METRICREPORTDEFINITIONCOLLECTION *)InterpProp;
  DestroyMetricReportDefinitionCollection_CS (MetricReportDefinitionCollection->MetricReportDefinitionCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from MetricReportDefinitionCollectionFromEfiStruct
...
EFI_STATUS
EFIAPI
MetricReportDefinitionCollectionDestoryJson(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
  IN EFI_REST_RESOURCE_TYPE_IDENTIFIER *RsrcIdentifier,  
  IN CHAR8 *ResoruceRaw
)
{
  ...
  return DestroyMetricReportDefinitionCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishMetricReportDefinitionCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishMetricReportDefinitionCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
