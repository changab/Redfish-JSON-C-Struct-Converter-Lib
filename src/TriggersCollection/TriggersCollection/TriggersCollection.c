...
#include"Redfish_TriggersCollection_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
//
static RedfishCS_status GenOemCs(RedfishTriggersCollection_TriggersCollection_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst)
{
...
//
// C structure to JSON for TriggersCollection.TriggersCollection
//
RedfishCS_status CS_To_TriggersCollection_JSON(RedfishTriggersCollection_TriggersCollection_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
...
//
// Destory C Structure for TriggersCollection.TriggersCollection
//
RedfishCS_status DestroyTriggersCollection_CS(RedfishTriggersCollection_TriggersCollection_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for TriggersCollection.TriggersCollection
//
RedfishCS_status DestroyTriggersCollection_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for TriggersCollection.NOVERSIONED.TriggersCollection
//
RedfishCS_status
Json_TriggersCollection_To_CS(RedfishCS_char *JsonRawText, RedfishTriggersCollection_TriggersCollection_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishTriggersCollection_TriggersCollection_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "TriggersCollection", "noversioned", "TriggersCollection", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishTriggersCollection_TriggersCollection_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
...
Error:;
  json_decref(JsonObj);
  DestroyTriggersCollection_CS (Cs);
  return Status;
}
