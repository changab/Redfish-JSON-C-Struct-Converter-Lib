...
//
#include <RedfishInterpreter/ProcessorCollection/EfiProcessorCollection.h>
CHAR8 mResourceTypeStr [] = "#ProcessorCollection";
BOOLEAN IsRevisonController = FALSE;

// Support ProcessorCollection  
...
EFI_STATUS
EFIAPI
ProcessorCollectionToEfiStruct (
...
{
  EFI_REDFISH_PROCESSORCOLLECTION *ProcessorCollection;
  EFI_REDFISH_PROCESSORCOLLECTION_CS *ProcessorCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "ProcessorCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_ProcessorCollection_To_CS (ResoruceRaw, &ProcessorCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ProcessorCollection = (EFI_REDFISH_PROCESSORCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_PROCESSORCOLLECTION));
  if (ProcessorCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)ProcessorCollection;
  ProcessorCollection->ProcessorCollection = ProcessorCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_PROCESSORCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("ProcessorCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "ProcessorCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("ProcessorCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "ProcessorCollection");
...
EFI_STATUS
EFIAPI
ProcessorCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_ProcessorCollection_JSON (*((EFI_REDFISH_PROCESSORCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in ProcessorCollectionToStruct.
...
EFI_STATUS
EFIAPI
ProcessorCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  ProcessorCollection = (EFI_REDFISH_PROCESSORCOLLECTION *)InterpProp;
  DestroyProcessorCollection_CS (ProcessorCollection->ProcessorCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from ProcessorCollectionFromEfiStruct
...
EFI_STATUS
EFIAPI
ProcessorCollectionDestoryJson(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
  IN EFI_REST_RESOURCE_TYPE_IDENTIFIER *RsrcIdentifier,  
  IN CHAR8 *ResoruceRaw
)
{
  ...
  return DestroyProcessorCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishProcessorCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishProcessorCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
