...
//
#include <RedfishInterpreter/EthernetInterfaceCollection/EfiEthernetInterfaceCollection.h>
CHAR8 mResourceTypeStr [] = "#EthernetInterfaceCollection";
BOOLEAN IsRevisonController = FALSE;

// Support EthernetInterfaceCollection  
...
EFI_STATUS
EFIAPI
EthernetInterfaceCollectionToEfiStruct (
...
{
  EFI_REDFISH_ETHERNETINTERFACECOLLECTION *EthernetInterfaceCollection;
  EFI_REDFISH_ETHERNETINTERFACECOLLECTION_CS *EthernetInterfaceCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "EthernetInterfaceCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_EthernetInterfaceCollection_To_CS (ResoruceRaw, &EthernetInterfaceCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  EthernetInterfaceCollection = (EFI_REDFISH_ETHERNETINTERFACECOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_ETHERNETINTERFACECOLLECTION));
  if (EthernetInterfaceCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)EthernetInterfaceCollection;
  EthernetInterfaceCollection->EthernetInterfaceCollection = EthernetInterfaceCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_ETHERNETINTERFACECOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("EthernetInterfaceCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "EthernetInterfaceCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("EthernetInterfaceCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "EthernetInterfaceCollection");
...
EFI_STATUS
EFIAPI
EthernetInterfaceCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_EthernetInterfaceCollection_JSON (*((EFI_REDFISH_ETHERNETINTERFACECOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in EthernetInterfaceCollectionToStruct.
...
EFI_STATUS
EFIAPI
EthernetInterfaceCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  EthernetInterfaceCollection = (EFI_REDFISH_ETHERNETINTERFACECOLLECTION *)InterpProp;
  DestroyEthernetInterfaceCollection_CS (EthernetInterfaceCollection->EthernetInterfaceCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from EthernetInterfaceCollectionFromEfiStruct
...
EFI_STATUS
EFIAPI
EthernetInterfaceCollectionDestoryJson(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
  IN EFI_REST_RESOURCE_TYPE_IDENTIFIER *RsrcIdentifier,  
  IN CHAR8 *ResoruceRaw
)
{
  ...
  return DestroyEthernetInterfaceCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishEthernetInterfaceCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishEthernetInterfaceCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
