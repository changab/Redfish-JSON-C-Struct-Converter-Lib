...
//
#include <RedfishInterpreter/LogServiceCollection/EfiLogServiceCollection.h>
CHAR8 mResourceTypeStr [] = "#LogServiceCollection";
...
EFI_STATUS
EFIAPI
LogServiceCollectionToEfiStruct (
...
{
  EFI_REDFISH_LOGSERVICECOLLECTION *LogServiceCollection;
  EFI_REDFISH_LOGSERVICECOLLECTION_CS *LogServiceCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "LogServiceCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_LogServiceCollection_To_CS (ResoruceRaw, &LogServiceCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  LogServiceCollection = (EFI_REDFISH_LOGSERVICECOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_LOGSERVICECOLLECTION));
  if (LogServiceCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)LogServiceCollection;
  LogServiceCollection->LogServiceCollection = LogServiceCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_LOGSERVICECOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("LogServiceCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "LogServiceCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("LogServiceCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "LogServiceCollection");
...
EFI_STATUS
EFIAPI
LogServiceCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_LogServiceCollection_JSON (*((EFI_REDFISH_LOGSERVICECOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in LogServiceCollectionToStruct.
...
EFI_STATUS
EFIAPI
LogServiceCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  LogServiceCollection = (EFI_REDFISH_LOGSERVICECOLLECTION *)InterpProp;
  DestroyLogServiceCollection_CS (LogServiceCollection->LogServiceCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from LogServiceCollectionFromEfiStruct
...
EFI_STATUS
EFIAPI
LogServiceCollectionDestoryJson(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
  IN EFI_REST_RESOURCE_TYPE_IDENTIFIER *RsrcIdentifier,  
  IN CHAR8 *ResoruceRaw
)
{
  ...
  return DestroyLogServiceCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishLogServiceCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishLogServiceCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
