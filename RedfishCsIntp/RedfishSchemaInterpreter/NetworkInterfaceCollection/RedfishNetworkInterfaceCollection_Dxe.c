...
//
#include <RedfishInterpreter/NetworkInterfaceCollection/EfiNetworkInterfaceCollection.h>
CHAR8 mResourceTypeStr [] = "#NetworkInterfaceCollection";
BOOLEAN IsRevisonController = FALSE;

// Support NetworkInterfaceCollection  
...
EFI_STATUS
EFIAPI
NetworkInterfaceCollectionToEfiStruct (
...
{
  EFI_REDFISH_NETWORKINTERFACECOLLECTION *NetworkInterfaceCollection;
  EFI_REDFISH_NETWORKINTERFACECOLLECTION_CS *NetworkInterfaceCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "NetworkInterfaceCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_NetworkInterfaceCollection_To_CS (ResoruceRaw, &NetworkInterfaceCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  NetworkInterfaceCollection = (EFI_REDFISH_NETWORKINTERFACECOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_NETWORKINTERFACECOLLECTION));
  if (NetworkInterfaceCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)NetworkInterfaceCollection;
  NetworkInterfaceCollection->NetworkInterfaceCollection = NetworkInterfaceCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_NETWORKINTERFACECOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("NetworkInterfaceCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "NetworkInterfaceCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("NetworkInterfaceCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "NetworkInterfaceCollection");
...
EFI_STATUS
EFIAPI
NetworkInterfaceCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_NetworkInterfaceCollection_JSON (*((EFI_REDFISH_NETWORKINTERFACECOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in NetworkInterfaceCollectionToStruct.
...
EFI_STATUS
EFIAPI
NetworkInterfaceCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  NetworkInterfaceCollection = (EFI_REDFISH_NETWORKINTERFACECOLLECTION *)InterpProp;
  DestroyNetworkInterfaceCollection_CS (NetworkInterfaceCollection->NetworkInterfaceCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from NetworkInterfaceCollectionFromEfiStruct
...
EFI_STATUS
EFIAPI
NetworkInterfaceCollectionDestoryJson(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
  IN EFI_REST_RESOURCE_TYPE_IDENTIFIER *RsrcIdentifier,  
  IN CHAR8 *ResoruceRaw
)
{
  ...
  return DestroyNetworkInterfaceCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishNetworkInterfaceCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishNetworkInterfaceCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
