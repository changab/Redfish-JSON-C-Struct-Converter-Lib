...
//
#include <RedfishInterpreter/ChassisCollection/EfiChassisCollection.h>
CHAR8 mResourceTypeStr [] = "#ChassisCollection";
BOOLEAN IsRevisonController = FALSE;

// Support ChassisCollection  
...
{
  EFI_REDFISH_CHASSISCOLLECTION *ChassisCollection;
  EFI_REDFISH_CHASSISCOLLECTION_CS *ChassisCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "ChassisCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_ChassisCollection_To_CS (ResoruceRaw, &ChassisCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ChassisCollection = (EFI_REDFISH_CHASSISCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_CHASSISCOLLECTION));
  if (ChassisCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)ChassisCollection;
  ChassisCollection->ChassisCollection = ChassisCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_CHASSISCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("ChassisCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "ChassisCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("ChassisCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "ChassisCollection");
...
EFI_STATUS
EFIAPI
ChassisCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_ChassisCollection_JSON (*((EFI_REDFISH_CHASSISCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in ChassisCollectionToStruct.
...
EFI_STATUS
EFIAPI
ChassisCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  ChassisCollection = (EFI_REDFISH_CHASSISCOLLECTION *)InterpProp;
  DestroyChassisCollection_CS (ChassisCollection->ChassisCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from ChassisCollectionFromEfiStruct
...
)
{
  ...
  return DestroyChassisCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishChassisCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishChassisCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
