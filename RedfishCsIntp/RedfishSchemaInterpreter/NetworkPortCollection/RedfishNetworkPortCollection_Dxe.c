...
//
#include <RedfishInterpreter/NetworkPortCollection/EfiNetworkPortCollection.h>
CHAR8 mResourceTypeStr [] = "#NetworkPortCollection";
...
EFI_STATUS
EFIAPI
NetworkPortCollectionToEfiStruct (
...
{
  EFI_REDFISH_NETWORKPORTCOLLECTION *NetworkPortCollection;
  EFI_REDFISH_NETWORKPORTCOLLECTION_CS *NetworkPortCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "NetworkPortCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_NetworkPortCollection_To_CS (ResoruceRaw, &NetworkPortCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  NetworkPortCollection = (EFI_REDFISH_NETWORKPORTCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_NETWORKPORTCOLLECTION));
  if (NetworkPortCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)NetworkPortCollection;
  NetworkPortCollection->NetworkPortCollection = NetworkPortCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_NETWORKPORTCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("NetworkPortCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "NetworkPortCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("NetworkPortCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "NetworkPortCollection");
...
EFI_STATUS
EFIAPI
NetworkPortCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_NetworkPortCollection_JSON (*((EFI_REDFISH_NETWORKPORTCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in NetworkPortCollectionToStruct.
...
EFI_STATUS
EFIAPI
NetworkPortCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  NetworkPortCollection = (EFI_REDFISH_NETWORKPORTCOLLECTION *)InterpProp;
  DestroyNetworkPortCollection_CS (NetworkPortCollection->NetworkPortCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from NetworkPortCollectionFromEfiStruct
...
EFI_STATUS
EFIAPI
NetworkPortCollectionDestoryJson(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
  IN EFI_REST_RESOURCE_TYPE_IDENTIFIER *RsrcIdentifier,  
  IN CHAR8 *ResoruceRaw
)
{
  ...
  return DestroyNetworkPortCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishNetworkPortCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishNetworkPortCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
