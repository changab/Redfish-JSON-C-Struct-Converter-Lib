...
//
#include <RedfishInterpreter/MessageRegistryCollection/EfiMessageRegistryCollection.h>
CHAR8 mResourceTypeStr [] = "#MessageRegistryCollection";
BOOLEAN IsRevisonController = FALSE;

// Support MessageRegistryCollection  
...
EFI_STATUS
EFIAPI
MessageRegistryCollectionToEfiStruct (
...
{
  EFI_REDFISH_MESSAGEREGISTRYCOLLECTION *MessageRegistryCollection;
  EFI_REDFISH_MESSAGEREGISTRYCOLLECTION_CS *MessageRegistryCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "MessageRegistryCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_MessageRegistryCollection_To_CS (ResoruceRaw, &MessageRegistryCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  MessageRegistryCollection = (EFI_REDFISH_MESSAGEREGISTRYCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_MESSAGEREGISTRYCOLLECTION));
  if (MessageRegistryCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)MessageRegistryCollection;
  MessageRegistryCollection->MessageRegistryCollection = MessageRegistryCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_MESSAGEREGISTRYCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("MessageRegistryCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "MessageRegistryCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("MessageRegistryCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "MessageRegistryCollection");
...
EFI_STATUS
EFIAPI
MessageRegistryCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_MessageRegistryCollection_JSON (*((EFI_REDFISH_MESSAGEREGISTRYCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in MessageRegistryCollectionToStruct.
...
EFI_STATUS
EFIAPI
MessageRegistryCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  MessageRegistryCollection = (EFI_REDFISH_MESSAGEREGISTRYCOLLECTION *)InterpProp;
  DestroyMessageRegistryCollection_CS (MessageRegistryCollection->MessageRegistryCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from MessageRegistryCollectionFromEfiStruct
...
EFI_STATUS
EFIAPI
MessageRegistryCollectionDestoryJson(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
  IN EFI_REST_RESOURCE_TYPE_IDENTIFIER *RsrcIdentifier,  
  IN CHAR8 *ResoruceRaw
)
{
  ...
  return DestroyMessageRegistryCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishMessageRegistryCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishMessageRegistryCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
