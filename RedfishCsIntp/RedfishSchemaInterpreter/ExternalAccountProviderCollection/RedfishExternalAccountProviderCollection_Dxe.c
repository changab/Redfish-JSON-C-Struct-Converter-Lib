...
//
#include <RedfishInterpreter/ExternalAccountProviderCollection/EfiExternalAccountProviderCollection.h>
CHAR8 mResourceTypeStr [] = "#ExternalAccountProviderCollection";
BOOLEAN IsRevisonController = FALSE;

// Support ExternalAccountProviderCollection  
...
EFI_STATUS
EFIAPI
ExternalAccountProviderCollectionToEfiStruct (
...
{
  EFI_REDFISH_EXTERNALACCOUNTPROVIDERCOLLECTION *ExternalAccountProviderCollection;
  EFI_REDFISH_EXTERNALACCOUNTPROVIDERCOLLECTION_CS *ExternalAccountProviderCollectionCs;
  ...
  } else if ((AsciiStrCmp (RsrcIdentifier->NameSpace.MajorVersion, "NOVERSIONED") != 0) ||
             (AsciiStrCmp (RsrcIdentifier->NameSpace.MinorVersion, "NOVERSIONED") != 0) ||
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ErrataVersion, "NOVERSIONED") != 0) ||
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "ExternalAccountProviderCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_ExternalAccountProviderCollection_To_CS (ResoruceRaw, &ExternalAccountProviderCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  ExternalAccountProviderCollection = (EFI_REDFISH_EXTERNALACCOUNTPROVIDERCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_EXTERNALACCOUNTPROVIDERCOLLECTION));
  if (ExternalAccountProviderCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)ExternalAccountProviderCollection;
  ExternalAccountProviderCollection->ExternalAccountProviderCollection = ExternalAccountProviderCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_EXTERNALACCOUNTPROVIDERCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("ExternalAccountProviderCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "ExternalAccountProviderCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("ExternalAccountProviderCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "ExternalAccountProviderCollection");
...
EFI_STATUS
EFIAPI
ExternalAccountProviderCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_ExternalAccountProviderCollection_JSON (*((EFI_REDFISH_EXTERNALACCOUNTPROVIDERCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in ExternalAccountProviderCollectionToStruct.
...
EFI_STATUS
EFIAPI
ExternalAccountProviderCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  ExternalAccountProviderCollection = (EFI_REDFISH_EXTERNALACCOUNTPROVIDERCOLLECTION *)InterpProp;
  DestroyExternalAccountProviderCollection_CS (ExternalAccountProviderCollection->ExternalAccountProviderCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from ExternalAccountProviderCollectionFromEfiStruct
...
EFI_STATUS
EFIAPI
ExternalAccountProviderCollectionDestoryJson(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
  IN EFI_REST_RESOURCE_TYPE_IDENTIFIER *RsrcIdentifier,  
  IN CHAR8 *ResoruceRaw
)
{
  ...
  return DestroyExternalAccountProviderCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishExternalAccountProviderCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishExternalAccountProviderCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
