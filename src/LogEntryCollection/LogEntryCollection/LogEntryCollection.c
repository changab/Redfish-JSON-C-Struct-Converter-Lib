...
#include"Redfish_LogEntryCollection_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
//
static RedfishCS_status GenOemCs(RedfishLogEntryCollection_LogEntryCollection_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst)
{
...
//
// C structure to JSON for LogEntryCollection.LogEntryCollection
//
RedfishCS_status CS_To_LogEntryCollection_JSON(RedfishLogEntryCollection_LogEntryCollection_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
...
//
// Destory C Structure for LogEntryCollection.LogEntryCollection
//
RedfishCS_status DestroyLogEntryCollection_CS(RedfishLogEntryCollection_LogEntryCollection_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for LogEntryCollection.LogEntryCollection
//
RedfishCS_status DestroyLogEntryCollection_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for LogEntryCollection.NOVERSIONED.LogEntryCollection
//
RedfishCS_status
Json_LogEntryCollection_To_CS(RedfishCS_char *JsonRawText, RedfishLogEntryCollection_LogEntryCollection_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishLogEntryCollection_LogEntryCollection_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "LogEntryCollection", "noversioned", "LogEntryCollection", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishLogEntryCollection_LogEntryCollection_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
...
Error:;
  json_decref(JsonObj);
  DestroyLogEntryCollection_CS (Cs);
  return Status;
}
