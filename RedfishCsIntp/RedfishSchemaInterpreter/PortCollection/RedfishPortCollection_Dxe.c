...
//
#include <RedfishInterpreter/PortCollection/EfiPortCollection.h>
CHAR8 mResourceTypeStr [] = "#PortCollection";
BOOLEAN IsRevisonController = FALSE;

// Support PortCollection  
...
{
  EFI_REDFISH_PORTCOLLECTION *PortCollection;
  EFI_REDFISH_PORTCOLLECTION_CS *PortCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "PortCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_PortCollection_To_CS (ResoruceRaw, &PortCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  PortCollection = (EFI_REDFISH_PORTCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_PORTCOLLECTION));
  if (PortCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)PortCollection;
  PortCollection->PortCollection = PortCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_PORTCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("PortCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "PortCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("PortCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "PortCollection");
...
)
{
  ...
  return CS_To_PortCollection_JSON (*((EFI_REDFISH_PORTCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in PortCollectionToStruct.
...
EFI_STATUS
EFIAPI
PortCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  PortCollection = (EFI_REDFISH_PORTCOLLECTION *)InterpProp;
  DestroyPortCollection_CS (PortCollection->PortCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from PortCollectionFromEfiStruct
...
)
{
  ...
  return DestroyPortCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishPortCollectionEntryPoint (
