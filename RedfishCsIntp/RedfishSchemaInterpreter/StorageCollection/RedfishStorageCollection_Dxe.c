...
//
#include <RedfishInterpreter/StorageCollection/EfiStorageCollection.h>
CHAR8 mResourceTypeStr [] = "#StorageCollection";
BOOLEAN IsRevisonController = FALSE;

// Support StorageCollection  
...
{
  EFI_REDFISH_STORAGECOLLECTION *StorageCollection;
  EFI_REDFISH_STORAGECOLLECTION_CS *StorageCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "StorageCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_StorageCollection_To_CS (ResoruceRaw, &StorageCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  StorageCollection = (EFI_REDFISH_STORAGECOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_STORAGECOLLECTION));
  if (StorageCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)StorageCollection;
  StorageCollection->StorageCollection = StorageCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_STORAGECOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("StorageCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "StorageCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("StorageCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "StorageCollection");
...
EFI_STATUS
EFIAPI
StorageCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_StorageCollection_JSON (*((EFI_REDFISH_STORAGECOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in StorageCollectionToStruct.
...
EFI_STATUS
EFIAPI
StorageCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  StorageCollection = (EFI_REDFISH_STORAGECOLLECTION *)InterpProp;
  DestroyStorageCollection_CS (StorageCollection->StorageCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from StorageCollectionFromEfiStruct
...
)
{
  ...
  return DestroyStorageCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishStorageCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishStorageCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
