...
//
#include <RedfishInterpreter/VirtualMediaCollection/EfiVirtualMediaCollection.h>
CHAR8 mResourceTypeStr [] = "#VirtualMediaCollection";
BOOLEAN IsRevisonController = FALSE;

// Support VirtualMediaCollection  
...
EFI_STATUS
EFIAPI
VirtualMediaCollectionToEfiStruct (
...
{
  EFI_REDFISH_VIRTUALMEDIACOLLECTION *VirtualMediaCollection;
  EFI_REDFISH_VIRTUALMEDIACOLLECTION_CS *VirtualMediaCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "VirtualMediaCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_VirtualMediaCollection_To_CS (ResoruceRaw, &VirtualMediaCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  VirtualMediaCollection = (EFI_REDFISH_VIRTUALMEDIACOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_VIRTUALMEDIACOLLECTION));
  if (VirtualMediaCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)VirtualMediaCollection;
  VirtualMediaCollection->VirtualMediaCollection = VirtualMediaCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_VIRTUALMEDIACOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("VirtualMediaCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "VirtualMediaCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("VirtualMediaCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "VirtualMediaCollection");
...
EFI_STATUS
EFIAPI
VirtualMediaCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_VirtualMediaCollection_JSON (*((EFI_REDFISH_VIRTUALMEDIACOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in VirtualMediaCollectionToStruct.
...
EFI_STATUS
EFIAPI
VirtualMediaCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  DestroyVirtualMediaCollection_CS (VirtualMediaCollection->VirtualMediaCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from VirtualMediaCollectionFromEfiStruct
...
EFI_STATUS
EFIAPI
VirtualMediaCollectionDestoryJson(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
  IN EFI_REST_RESOURCE_TYPE_IDENTIFIER *RsrcIdentifier,  
  IN CHAR8 *ResoruceRaw
)
{
  ...
  return DestroyVirtualMediaCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishVirtualMediaCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishVirtualMediaCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
