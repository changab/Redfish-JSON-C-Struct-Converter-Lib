...
#include"Redfish_Zone_v1_1_0_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
static RedfishCS_status GenOemCs(RedfishZone_V1_1_0_Zone_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst);
//
//Generate C structure for Oem
//
static RedfishCS_status GenActions_OemCs(RedfishZone_V1_1_0_Zone_CS *Cs, json_t *JsonObj, char *Key, RedfishZone_V1_1_0_Zone_Actions_Oem_CS **Dst)
{
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishZone_V1_1_0_Zone_Actions_Oem_CS), (RedfishCS_void **)Dst);
...
//
static RedfishCS_status GenActionsCs(RedfishZone_V1_1_0_Zone_CS *Cs, json_t *JsonObj, char *Key, RedfishZone_V1_1_0_Zone_Actions_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishZone_V1_1_0_Zone_Actions_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // Oem
...
//
static RedfishCS_status GenLinksCs(RedfishZone_V1_1_0_Zone_CS *Cs, json_t *JsonObj, char *Key, RedfishZone_V1_1_0_Zone_Links_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishZone_V1_1_0_Zone_Links_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // Endpoints
  ...
  // InvolvedSwitches
  InitializeLinkHead (&(*Dst)->InvolvedSwitches);
  Status = CreateCsUriOrJsonByNodeArray (Cs, TempJsonObj, "InvolvedSwitches", Cs->Header.ThisUri, &(*Dst)->InvolvedSwitches);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // InvolvedSwitches@odata.count
  Status = GetRedfishPropertyInt64 (Cs, TempJsonObj, "InvolvedSwitches@odata.count", &(*Dst)->InvolvedSwitchesodata_count);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // InvolvedSwitches@odata.navigationLink
  Status = GetRedfishPropertyStr (Cs, TempJsonObj, "InvolvedSwitches@odata.navigationLink", &(*Dst)->InvolvedSwitchesodata_navigationLink);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
...
//
static RedfishCS_status GenStatusCs(RedfishZone_V1_1_0_Zone_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Status_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
...
static RedfishCS_status CS_To_JSON_ActionsOem(json_t *CsJson, char *Key, RedfishZone_V1_1_0_Zone_Actions_Oem_CS *CSPtr)
{
  if (CSPtr == NULL) {
...
static RedfishCS_status CS_To_JSON_Actions(json_t *CsJson, char *Key, RedfishZone_V1_1_0_Zone_Actions_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
static RedfishCS_status CS_To_JSON_Links(json_t *CsJson, char *Key, RedfishZone_V1_1_0_Zone_Links_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
  ...
  // InvolvedSwitches 
  if (InsertJsonLinkArrayObj (CsJson, "InvolvedSwitches", &CSPtr->InvolvedSwitches) != RedfishCS_status_success) {goto Error;}

  // InvolvedSwitches@odata.count 
  if (InsertJsonInt64Obj (CsJson, "InvolvedSwitches@odata.count", CSPtr->InvolvedSwitchesodata_count) != RedfishCS_status_success) {goto Error;}

  // InvolvedSwitches@odata.navigationLink 
  if (InsertJsonStringObj (CsJson, "InvolvedSwitches@odata.navigationLink", CSPtr->InvolvedSwitchesodata_navigationLink) != RedfishCS_status_success) {goto Error;}
...
//
// C structure to JSON for Zone.V1_1_0.Zone
//
RedfishCS_status CS_To_Zone_V1_1_0_JSON(RedfishZone_V1_1_0_Zone_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
...
//
// Destory C Structure for Zone.V1_1_0.Zone
//
RedfishCS_status DestroyZone_V1_1_0_CS(RedfishZone_V1_1_0_Zone_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for Zone.V1_1_0.Zone
//
RedfishCS_status DestroyZone_V1_1_0_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for Zone.V1_1_0.Zone
//
RedfishCS_status
Json_Zone_V1_1_0_To_CS(RedfishCS_char *JsonRawText, RedfishZone_V1_1_0_Zone_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishZone_V1_1_0_Zone_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "Zone", "v1_1_0", "Zone", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishZone_V1_1_0_Zone_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
