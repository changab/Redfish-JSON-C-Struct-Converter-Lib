...
#include"Redfish_LogServiceCollection_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
//
static RedfishCS_status GenOemCs(RedfishLogServiceCollection_LogServiceCollection_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst)
{
...
//
// C structure to JSON for LogServiceCollection.LogServiceCollection
//
RedfishCS_status CS_To_LogServiceCollection_JSON(RedfishLogServiceCollection_LogServiceCollection_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
...
//
// Destory C Structure for LogServiceCollection.LogServiceCollection
//
RedfishCS_status DestroyLogServiceCollection_CS(RedfishLogServiceCollection_LogServiceCollection_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for LogServiceCollection.LogServiceCollection
//
RedfishCS_status DestroyLogServiceCollection_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for LogServiceCollection.NOVERSIONED.LogServiceCollection
//
RedfishCS_status
Json_LogServiceCollection_To_CS(RedfishCS_char *JsonRawText, RedfishLogServiceCollection_LogServiceCollection_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishLogServiceCollection_LogServiceCollection_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "LogServiceCollection", "noversioned", "LogServiceCollection", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishLogServiceCollection_LogServiceCollection_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
...
Error:;
  json_decref(JsonObj);
  DestroyLogServiceCollection_CS (Cs);
  return Status;
}
