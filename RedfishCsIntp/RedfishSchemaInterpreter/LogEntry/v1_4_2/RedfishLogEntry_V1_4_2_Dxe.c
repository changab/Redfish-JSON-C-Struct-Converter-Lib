...
//
#include <RedfishInterpreter/LogEntry/v1_4_2/EfiLogEntryV1_4_2.h>
CHAR8 mResourceTypeStr [] = "#LogEntry";
BOOLEAN IsRevisonController = TRUE;

// Support LogEntry V1_4_2 
...
{
  EFI_REDFISH_LOGENTRY_V1_4_2 *LogEntryV1_4_2;
  EFI_REDFISH_LOGENTRY_V1_4_2_CS *LogEntryV1_4_2Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "LogEntry") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_LogEntry_V1_4_2_To_CS (ResoruceRaw, &LogEntryV1_4_2Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  LogEntryV1_4_2 = (EFI_REDFISH_LOGENTRY_V1_4_2 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_LOGENTRY_V1_4_2));
  if (LogEntryV1_4_2 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_JSON_STRUCTURE_HEADER *)LogEntryV1_4_2;
  LogEntryV1_4_2->LogEntry = LogEntryV1_4_2Cs;
  ResourceTypeMajorVersion = "1";
  ResourceTypeMinorVersion = "4";
  ResourceTypeErrataVersion = "2";
  Length = sizeof (EFI_REDFISH_LOGENTRY_V1_4_2_CS);
  //
  // Set header.
  //
  RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("LogEntry"));
  AsciiStrCpy (RestIntrpHeader->JsonRsrcIdentifier.NameSpace.ResourceTypeName, "LogEntry");
  ...
  RestIntrpHeader->JsonRsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("LogEntry"));
  AsciiStrCpy (RestIntrpHeader->JsonRsrcIdentifier.DataType, "LogEntry");
...
/**
  The wrapper function of LogEntryToStruct which invokes **RESOURCE_TYPE**!ToStruct
...
)
{
  ...
  return CS_To_LogEntry_V1_4_2_JSON (*((EFI_REDFISH_LOGENTRY_V1_4_2_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_JSON_STRUCTURE_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in LogEntryToStruct.
...
)
{
  ...
  LogEntryV1_4_2 = (EFI_REDFISH_LOGENTRY_V1_4_2 *)InterpProp;
  DestroyLogEntry_V1_4_2_CS (LogEntryV1_4_2->LogEntry); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from LogEntryFromStruct
...
)
{
  ...
  return DestroyLogEntry_V1_4_2_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishLogEntry_V1_4_2EntryPoint (
