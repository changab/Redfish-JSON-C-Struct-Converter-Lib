...
#include"Redfish_TelemetryService_v1_0_0_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
static RedfishCS_status GenOemCs(RedfishTelemetryService_V1_0_0_TelemetryService_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst);
//
//Generate C structure for #TelemetryService.SubmitTestMetricReport
//
static RedfishCS_status GenSubmitTestMetricReportCs(RedfishTelemetryService_V1_0_0_TelemetryService_CS *Cs, json_t *JsonObj, char *Key, RedfishTelemetryService_V1_0_0_SubmitTestMetricReport_CS **Dst)
{
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishTelemetryService_V1_0_0_SubmitTestMetricReport_CS), (RedfishCS_void **)Dst);
...
//
static RedfishCS_status GenOemActionsCs(RedfishTelemetryService_V1_0_0_TelemetryService_CS *Cs, json_t *JsonObj, char *Key, RedfishTelemetryService_V1_0_0_OemActions_CS **Dst)
{
  RedfishCS_status Status;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishTelemetryService_V1_0_0_OemActions_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
...
//
static RedfishCS_status GenActionsCs(RedfishTelemetryService_V1_0_0_TelemetryService_CS *Cs, json_t *JsonObj, char *Key, RedfishTelemetryService_V1_0_0_Actions_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishTelemetryService_V1_0_0_Actions_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // #TelemetryService.SubmitTestMetricReport
  Status = GenSubmitTestMetricReportCs (Cs, TempJsonObj, "#TelemetryService.SubmitTestMetricReport", &(*Dst)->TelemetryService_SubmitTestMetricReport);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
...
//
static RedfishCS_status GenStatusCs(RedfishTelemetryService_V1_0_0_TelemetryService_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Status_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
...
//
static RedfishCS_status GenRedfishCS_char_ArrayCs(RedfishTelemetryService_V1_0_0_TelemetryService_CS *Cs, json_t *JsonObj, char *Key, RedfishCS_char_Array **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
...
static RedfishCS_status CS_To_JSON_ActionsOem(json_t *CsJson, char *Key, RedfishTelemetryService_V1_0_0_OemActions_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
static RedfishCS_status CS_To_JSON_ActionsTelemetryService_SubmitTestMetricReport(json_t *CsJson, char *Key, RedfishTelemetryService_V1_0_0_SubmitTestMetricReport_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
static RedfishCS_status CS_To_JSON_Actions(json_t *CsJson, char *Key, RedfishTelemetryService_V1_0_0_Actions_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
  ...
  // #TelemetryService.SubmitTestMetricReport
  if (CS_To_JSON_ActionsTelemetryService_SubmitTestMetricReport(CsJson, "#TelemetryService.SubmitTestMetricReport", CSPtr->TelemetryService_SubmitTestMetricReport) != RedfishCS_status_success) {goto Error;}
...
//
// C structure to JSON for TelemetryService.V1_0_0.TelemetryService
//
RedfishCS_status CS_To_TelemetryService_V1_0_0_JSON(RedfishTelemetryService_V1_0_0_TelemetryService_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
  ...
  // LogService 
  if (InsertJsonLinkObj (CsJson, "LogService", &CSPtr->LogService) != RedfishCS_status_success) {goto Error;}

  // MaxReports 
  if (InsertJsonInt64Obj (CsJson, "MaxReports", CSPtr->MaxReports) != RedfishCS_status_success) {goto Error;}

  // MetricDefinitions 
  if (InsertJsonLinkObj (CsJson, "MetricDefinitions", &CSPtr->MetricDefinitions) != RedfishCS_status_success) {goto Error;}

  // MetricReportDefinitions 
  if (InsertJsonLinkObj (CsJson, "MetricReportDefinitions", &CSPtr->MetricReportDefinitions) != RedfishCS_status_success) {goto Error;}

  // MetricReports 
  if (InsertJsonLinkObj (CsJson, "MetricReports", &CSPtr->MetricReports) != RedfishCS_status_success) {goto Error;}

  // MinCollectionInterval 
  if (InsertJsonStringObj (CsJson, "MinCollectionInterval", CSPtr->MinCollectionInterval) != RedfishCS_status_success) {goto Error;}
  ...
  if (InsertJsonStringArrayObj (CsJson, "SupportedCollectionFunctions", CSPtr->SupportedCollectionFunctions) != RedfishCS_status_success) {goto Error;}

  // Triggers 
  if (InsertJsonLinkObj (CsJson, "Triggers", &CSPtr->Triggers) != RedfishCS_status_success) {goto Error;}
...
//
// Destory C Structure for TelemetryService.V1_0_0.TelemetryService
//
RedfishCS_status DestroyTelemetryService_V1_0_0_CS(RedfishTelemetryService_V1_0_0_TelemetryService_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for TelemetryService.V1_0_0.TelemetryService
//
RedfishCS_status DestroyTelemetryService_V1_0_0_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for TelemetryService.V1_0_0.TelemetryService
//
RedfishCS_status
Json_TelemetryService_V1_0_0_To_CS(RedfishCS_char *JsonRawText, RedfishTelemetryService_V1_0_0_TelemetryService_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishTelemetryService_V1_0_0_TelemetryService_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "TelemetryService", "v1_0_0", "TelemetryService", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishTelemetryService_V1_0_0_TelemetryService_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
  ...
  // LogService
  InitializeLinkHead (&Cs->LogService);
  Status = CreateCsUriOrJsonByNode (Cs, JsonObj, "LogService", Cs->Header.ThisUri, &Cs->LogService);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // MaxReports
  Status = GetRedfishPropertyInt64 (Cs, JsonObj, "MaxReports", &Cs->MaxReports);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // MetricDefinitions
  InitializeLinkHead (&Cs->MetricDefinitions);
  Status = CreateCsUriOrJsonByNode (Cs, JsonObj, "MetricDefinitions", Cs->Header.ThisUri, &Cs->MetricDefinitions);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // MetricReportDefinitions
  InitializeLinkHead (&Cs->MetricReportDefinitions);
  Status = CreateCsUriOrJsonByNode (Cs, JsonObj, "MetricReportDefinitions", Cs->Header.ThisUri, &Cs->MetricReportDefinitions);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // MetricReports
  InitializeLinkHead (&Cs->MetricReports);
  Status = CreateCsUriOrJsonByNode (Cs, JsonObj, "MetricReports", Cs->Header.ThisUri, &Cs->MetricReports);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // MinCollectionInterval
  Status = GetRedfishPropertyStr (Cs, JsonObj, "MinCollectionInterval", &Cs->MinCollectionInterval);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
  ...
  // SupportedCollectionFunctions
  Status = GenRedfishCS_char_ArrayCs (Cs, JsonObj, "SupportedCollectionFunctions", &Cs->SupportedCollectionFunctions);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // Triggers
  InitializeLinkHead (&Cs->Triggers);
  Status = CreateCsUriOrJsonByNode (Cs, JsonObj, "Triggers", Cs->Header.ThisUri, &Cs->Triggers);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
...
Error:;
  json_decref(JsonObj);
  DestroyTelemetryService_V1_0_0_CS (Cs);
  return Status;
}
