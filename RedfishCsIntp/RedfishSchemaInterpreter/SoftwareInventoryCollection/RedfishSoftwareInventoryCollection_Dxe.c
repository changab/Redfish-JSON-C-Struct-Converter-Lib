...
//
#include <RedfishInterpreter/SoftwareInventoryCollection/EfiSoftwareInventoryCollection.h>
CHAR8 mResourceTypeStr [] = "#SoftwareInventoryCollection";
BOOLEAN IsRevisonController = FALSE;

// Support SoftwareInventoryCollection  
...
EFI_STATUS
EFIAPI
SoftwareInventoryCollectionToEfiStruct (
...
{
  EFI_REDFISH_SOFTWAREINVENTORYCOLLECTION *SoftwareInventoryCollection;
  EFI_REDFISH_SOFTWAREINVENTORYCOLLECTION_CS *SoftwareInventoryCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "SoftwareInventoryCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_SoftwareInventoryCollection_To_CS (ResoruceRaw, &SoftwareInventoryCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  SoftwareInventoryCollection = (EFI_REDFISH_SOFTWAREINVENTORYCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_SOFTWAREINVENTORYCOLLECTION));
  if (SoftwareInventoryCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)SoftwareInventoryCollection;
  SoftwareInventoryCollection->SoftwareInventoryCollection = SoftwareInventoryCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_SOFTWAREINVENTORYCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("SoftwareInventoryCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "SoftwareInventoryCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("SoftwareInventoryCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "SoftwareInventoryCollection");
...
EFI_STATUS
EFIAPI
SoftwareInventoryCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_SoftwareInventoryCollection_JSON (*((EFI_REDFISH_SOFTWAREINVENTORYCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in SoftwareInventoryCollectionToStruct.
...
EFI_STATUS
EFIAPI
SoftwareInventoryCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  SoftwareInventoryCollection = (EFI_REDFISH_SOFTWAREINVENTORYCOLLECTION *)InterpProp;
  DestroySoftwareInventoryCollection_CS (SoftwareInventoryCollection->SoftwareInventoryCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from SoftwareInventoryCollectionFromEfiStruct
...
EFI_STATUS
EFIAPI
SoftwareInventoryCollectionDestoryJson(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
  IN EFI_REST_RESOURCE_TYPE_IDENTIFIER *RsrcIdentifier,  
  IN CHAR8 *ResoruceRaw
)
{
  ...
  return DestroySoftwareInventoryCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishSoftwareInventoryCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishSoftwareInventoryCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
