...
//
#include <RedfishInterpreter/JobCollection/EfiJobCollection.h>
CHAR8 mResourceTypeStr [] = "#JobCollection";
BOOLEAN IsRevisonController = FALSE;

// Support JobCollection  
...
{
  EFI_REDFISH_JOBCOLLECTION *JobCollection;
  EFI_REDFISH_JOBCOLLECTION_CS *JobCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "JobCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_JobCollection_To_CS (ResoruceRaw, &JobCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  JobCollection = (EFI_REDFISH_JOBCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_JOBCOLLECTION));
  if (JobCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)JobCollection;
  JobCollection->JobCollection = JobCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_JOBCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("JobCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "JobCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("JobCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "JobCollection");
...
)
{
  ...
  return CS_To_JobCollection_JSON (*((EFI_REDFISH_JOBCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in JobCollectionToStruct.
...
)
{
  ...
  JobCollection = (EFI_REDFISH_JOBCOLLECTION *)InterpProp;
  DestroyJobCollection_CS (JobCollection->JobCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from JobCollectionFromEfiStruct
...
)
{
  ...
  return DestroyJobCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishJobCollectionEntryPoint (
