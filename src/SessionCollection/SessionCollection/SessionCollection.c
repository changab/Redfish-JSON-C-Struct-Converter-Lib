...
#include"Redfish_SessionCollection_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
//
static RedfishCS_status GenOemCs(RedfishSessionCollection_SessionCollection_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst)
{
...
//
// C structure to JSON for SessionCollection.SessionCollection
//
RedfishCS_status CS_To_SessionCollection_JSON(RedfishSessionCollection_SessionCollection_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
...
//
// Destory C Structure for SessionCollection.SessionCollection
//
RedfishCS_status DestroySessionCollection_CS(RedfishSessionCollection_SessionCollection_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for SessionCollection.SessionCollection
//
RedfishCS_status DestroySessionCollection_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for SessionCollection.NOVERSIONED.SessionCollection
//
RedfishCS_status
Json_SessionCollection_To_CS(RedfishCS_char *JsonRawText, RedfishSessionCollection_SessionCollection_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishSessionCollection_SessionCollection_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "SessionCollection", "noversioned", "SessionCollection", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishSessionCollection_SessionCollection_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
...
Error:;
  json_decref(JsonObj);
  DestroySessionCollection_CS (Cs);
  return Status;
}
