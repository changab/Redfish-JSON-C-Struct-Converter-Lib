...
//
#include <RedfishInterpreter/Power/v1_2_6/EfiPowerV1_2_6.h>
CHAR8 mResourceTypeStr [] = "#Power";
BOOLEAN IsRevisonController = TRUE;

// Support Power V1_2_6 
...
{
  EFI_REDFISH_POWER_V1_2_6 *PowerV1_2_6;
  EFI_REDFISH_POWER_V1_2_6_CS *PowerV1_2_6Cs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "Power") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_Power_V1_2_6_To_CS (ResoruceRaw, &PowerV1_2_6Cs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  PowerV1_2_6 = (EFI_REDFISH_POWER_V1_2_6 *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_POWER_V1_2_6));
  if (PowerV1_2_6 == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)PowerV1_2_6;
  PowerV1_2_6->Power = PowerV1_2_6Cs;
  ResourceTypeMajorVersion = "1";
  ResourceTypeMinorVersion = "2";
  ResourceTypeErrataVersion = "6";
  Length = sizeof (EFI_REDFISH_POWER_V1_2_6_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("Power"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "Power");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("Power"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "Power");
...
)
{
  ...
  return CS_To_Power_V1_2_6_JSON (*((EFI_REDFISH_POWER_V1_2_6_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in PowerToStruct.
...
)
{
  ...
  PowerV1_2_6 = (EFI_REDFISH_POWER_V1_2_6 *)InterpProp;
  DestroyPower_V1_2_6_CS (PowerV1_2_6->Power); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from PowerFromEfiStruct
...
)
{
  ...
  return DestroyPower_V1_2_6_Json (ResoruceRaw);
}
