...
#include"Redfish_SerialInterface_v1_1_3_CS.h"
#include <stdlib.h>
#include <string.h>
#include <jansson.h>
...
//
static RedfishCS_status GenOemActionsCs(RedfishSerialInterface_V1_1_3_SerialInterface_CS *Cs, json_t *JsonObj, char *Key, RedfishSerialInterface_V1_1_3_OemActions_CS **Dst)
{
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishSerialInterface_V1_1_3_OemActions_CS), (RedfishCS_void **)Dst);
...
//
static RedfishCS_status GenActionsCs(RedfishSerialInterface_V1_1_3_SerialInterface_CS *Cs, json_t *JsonObj, char *Key, RedfishSerialInterface_V1_1_3_Actions_CS **Dst)
{
  RedfishCS_status Status;
  json_t *TempJsonObj;
  ...
  Status = allocateRecordCsMemory(Cs, sizeof(RedfishSerialInterface_V1_1_3_Actions_CS), (RedfishCS_void **)Dst);
  if (Status != RedfishCS_status_success){
    goto Error;
  }
  // Oem
...
//Generate C structure for Oem
//
static RedfishCS_status GenOemCs(RedfishSerialInterface_V1_1_3_SerialInterface_CS *Cs, json_t *JsonObj, char *Key, RedfishResource_Oem_CS **Dst)
{
  RedfishCS_status Status;
  RedfishCS_Type_JSON_Data *CsTypeJson;
  RedfishCS_Type_EmptyProp_CS_Data *CsTypeEmptyPropCS;
...
static RedfishCS_status CS_To_JSON_ActionsOem(json_t *CsJson, char *Key, RedfishSerialInterface_V1_1_3_OemActions_CS *CSPtr)
{
  if (CSPtr == NULL) {
...
static RedfishCS_status CS_To_JSON_Actions(json_t *CsJson, char *Key, RedfishSerialInterface_V1_1_3_Actions_CS *CSPtr)
{
  if (CSPtr == NULL) {
    return RedfishCS_status_success;
  }
...
//
// C structure to JSON for SerialInterface.V1_1_3.SerialInterface
//
RedfishCS_status CS_To_SerialInterface_V1_1_3_JSON(RedfishSerialInterface_V1_1_3_SerialInterface_CS *CSPtr, RedfishCS_char **JsonText)
{
  json_t  *CsJson;
  ...
  // BitRate 
  if (InsertJsonStringObj (CsJson, "BitRate", CSPtr->BitRate) != RedfishCS_status_success) {goto Error;}

  // ConnectorType 
  if (InsertJsonStringObj (CsJson, "ConnectorType", CSPtr->ConnectorType) != RedfishCS_status_success) {goto Error;}

  // DataBits 
  if (InsertJsonStringObj (CsJson, "DataBits", CSPtr->DataBits) != RedfishCS_status_success) {goto Error;}

  // Description 
  if (InsertJsonStringObj (CsJson, "Description", CSPtr->Description) != RedfishCS_status_success) {goto Error;}

  // FlowControl 
  if (InsertJsonStringObj (CsJson, "FlowControl", CSPtr->FlowControl) != RedfishCS_status_success) {goto Error;}
  ...
  // Parity 
  if (InsertJsonStringObj (CsJson, "Parity", CSPtr->Parity) != RedfishCS_status_success) {goto Error;}

  // PinOut 
  if (InsertJsonStringObj (CsJson, "PinOut", CSPtr->PinOut) != RedfishCS_status_success) {goto Error;}

  // SignalType 
  if (InsertJsonStringObj (CsJson, "SignalType", CSPtr->SignalType) != RedfishCS_status_success) {goto Error;}

  // StopBits 
  if (InsertJsonStringObj (CsJson, "StopBits", CSPtr->StopBits) != RedfishCS_status_success) {goto Error;}
...
//
// Destory C Structure for SerialInterface.V1_1_3.SerialInterface
//
RedfishCS_status DestroySerialInterface_V1_1_3_CS(RedfishSerialInterface_V1_1_3_SerialInterface_CS *CSPtr)
{
  RedfishCS_status Status;
...
//
// Destory JSON text for SerialInterface.V1_1_3.SerialInterface
//
RedfishCS_status DestroySerialInterface_V1_1_3_Json(RedfishCS_char *JsonText)
{
...
//
//Generate C structure for SerialInterface.V1_1_3.SerialInterface
//
RedfishCS_status
Json_SerialInterface_V1_1_3_To_CS(RedfishCS_char *JsonRawText, RedfishSerialInterface_V1_1_3_SerialInterface_CS **ReturnedCs)
{
  RedfishCS_status  Status;
  json_t *JsonObj;
  RedfishSerialInterface_V1_1_3_SerialInterface_CS *Cs;

  Status = CreateJsonPayloadAndCs (JsonRawText, "SerialInterface", "v1_1_3", "SerialInterface", &JsonObj, (RedfishCS_void **)&Cs, sizeof (RedfishSerialInterface_V1_1_3_SerialInterface_CS));
  if (Status != RedfishCS_status_success) {
    goto Error;
  }
  ...
  // BitRate
  Status = GetRedfishPropertyStr (Cs, JsonObj, "BitRate", &Cs->BitRate);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // ConnectorType
  Status = GetRedfishPropertyStr (Cs, JsonObj, "ConnectorType", &Cs->ConnectorType);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // DataBits
  Status = GetRedfishPropertyStr (Cs, JsonObj, "DataBits", &Cs->DataBits);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
  ...
  // FlowControl
  Status = GetRedfishPropertyStr (Cs, JsonObj, "FlowControl", &Cs->FlowControl);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
  ...
  // Parity
  Status = GetRedfishPropertyStr (Cs, JsonObj, "Parity", &Cs->Parity);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // PinOut
  Status = GetRedfishPropertyStr (Cs, JsonObj, "PinOut", &Cs->PinOut);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // SignalType
  Status = GetRedfishPropertyStr (Cs, JsonObj, "SignalType", &Cs->SignalType);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}

  // StopBits
  Status = GetRedfishPropertyStr (Cs, JsonObj, "StopBits", &Cs->StopBits);
  if (Status != RedfishCS_status_success && Status != RedfishCS_status_not_found) {goto Error;}
  else {if (Status == RedfishCS_status_not_found){/*This is not the required property.*/}}
...
Error:;
  json_decref(JsonObj);
  DestroySerialInterface_V1_1_3_CS (Cs);
  return Status;
}
