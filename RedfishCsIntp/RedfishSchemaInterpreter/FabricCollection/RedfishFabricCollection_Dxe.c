...
//
#include <RedfishInterpreter/FabricCollection/EfiFabricCollection.h>
CHAR8 mResourceTypeStr [] = "#FabricCollection";
...
{
  EFI_REDFISH_FABRICCOLLECTION *FabricCollection;
  EFI_REDFISH_FABRICCOLLECTION_CS *FabricCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "FabricCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_FabricCollection_To_CS (ResoruceRaw, &FabricCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  FabricCollection = (EFI_REDFISH_FABRICCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_FABRICCOLLECTION));
  if (FabricCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)FabricCollection;
  FabricCollection->FabricCollection = FabricCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_FABRICCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("FabricCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "FabricCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("FabricCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "FabricCollection");
...
)
{
  ...
  return CS_To_FabricCollection_JSON (*((EFI_REDFISH_FABRICCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in FabricCollectionToStruct.
...
EFI_STATUS
EFIAPI
FabricCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  FabricCollection = (EFI_REDFISH_FABRICCOLLECTION *)InterpProp;
  DestroyFabricCollection_CS (FabricCollection->FabricCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from FabricCollectionFromEfiStruct
...
)
{
  ...
  return DestroyFabricCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishFabricCollectionEntryPoint (
