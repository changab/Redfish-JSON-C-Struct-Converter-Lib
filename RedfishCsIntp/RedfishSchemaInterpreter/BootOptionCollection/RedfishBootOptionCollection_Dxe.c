...
//
#include <RedfishInterpreter/BootOptionCollection/EfiBootOptionCollection.h>
CHAR8 mResourceTypeStr [] = "#BootOptionCollection";
BOOLEAN IsRevisonController = FALSE;

// Support BootOptionCollection  
...
EFI_STATUS
EFIAPI
BootOptionCollectionToEfiStruct (
...
{
  EFI_REDFISH_BOOTOPTIONCOLLECTION *BootOptionCollection;
  EFI_REDFISH_BOOTOPTIONCOLLECTION_CS *BootOptionCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "BootOptionCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_BootOptionCollection_To_CS (ResoruceRaw, &BootOptionCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  BootOptionCollection = (EFI_REDFISH_BOOTOPTIONCOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_BOOTOPTIONCOLLECTION));
  if (BootOptionCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)BootOptionCollection;
  BootOptionCollection->BootOptionCollection = BootOptionCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_BOOTOPTIONCOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("BootOptionCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "BootOptionCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("BootOptionCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "BootOptionCollection");
...
EFI_STATUS
EFIAPI
BootOptionCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_BootOptionCollection_JSON (*((EFI_REDFISH_BOOTOPTIONCOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in BootOptionCollectionToStruct.
...
EFI_STATUS
EFIAPI
BootOptionCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  BootOptionCollection = (EFI_REDFISH_BOOTOPTIONCOLLECTION *)InterpProp;
  DestroyBootOptionCollection_CS (BootOptionCollection->BootOptionCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from BootOptionCollectionFromEfiStruct
...
EFI_STATUS
EFIAPI
BootOptionCollectionDestoryJson(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
  IN EFI_REST_RESOURCE_TYPE_IDENTIFIER *RsrcIdentifier,  
  IN CHAR8 *ResoruceRaw
)
{
  ...
  return DestroyBootOptionCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishBootOptionCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishBootOptionCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
