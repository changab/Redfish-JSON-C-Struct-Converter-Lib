...
//
#include <RedfishInterpreter/MessageRegistryFileCollection/EfiMessageRegistryFileCollection.h>
CHAR8 mResourceTypeStr [] = "#MessageRegistryFileCollection";
BOOLEAN IsRevisonController = FALSE;

// Support MessageRegistryFileCollection  
...
EFI_STATUS
EFIAPI
MessageRegistryFileCollectionToEfiStruct (
...
{
  EFI_REDFISH_MESSAGEREGISTRYFILECOLLECTION *MessageRegistryFileCollection;
  EFI_REDFISH_MESSAGEREGISTRYFILECOLLECTION_CS *MessageRegistryFileCollectionCs;
  ...
  if (RsrcIdentifier == NULL ||
    ...
    if (!CheckSupportedSchema (ResoruceRaw)) {
      return EFI_UNSUPPORTED;
             ...
             (AsciiStrCmp (RsrcIdentifier->NameSpace.ResourceTypeName, "MessageRegistryFileCollection") != 0)) {
    return EFI_UNSUPPORTED;
  }

  Status = Json_MessageRegistryFileCollection_To_CS (ResoruceRaw, &MessageRegistryFileCollectionCs);
  if (EFI_ERROR (Status)){
    return Status;
  }
  MessageRegistryFileCollection = (EFI_REDFISH_MESSAGEREGISTRYFILECOLLECTION *)AllocateZeroPool (
                        sizeof (EFI_REDFISH_MESSAGEREGISTRYFILECOLLECTION));
  if (MessageRegistryFileCollection == NULL){
    return EFI_OUT_OF_RESOURCES;
  }
  RestIntrpHeader = (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER *)MessageRegistryFileCollection;
  MessageRegistryFileCollection->MessageRegistryFileCollection = MessageRegistryFileCollectionCs;
  ResourceTypeMajorVersion = "NOVERSIONED";
  ResourceTypeMinorVersion = "NOVERSIONED";
  ResourceTypeErrataVersion = "NOVERSIONED";
  Length = sizeof (EFI_REDFISH_MESSAGEREGISTRYFILECOLLECTION_CS);
  //
  // Set header.
  //
  RestIntrpHeader->Signature = REST_RESOURCE_INTERP_SIGNATURE; 
  RestIntrpHeader->Length = Length;
  RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName = (CHAR8 *)AllocatePool (AsciiStrSize ("MessageRegistryFileCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.NameSpace.ResourceTypeName, "MessageRegistryFileCollection");
  ...
  RestIntrpHeader->RsrcIdentifier.DataType = (CHAR8 *)AllocatePool (AsciiStrSize ("MessageRegistryFileCollection"));
  AsciiStrCpy (RestIntrpHeader->RsrcIdentifier.DataType, "MessageRegistryFileCollection");
...
EFI_STATUS
EFIAPI
MessageRegistryFileCollectionFromEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
...
)
{
  ...
  return CS_To_MessageRegistryFileCollection_JSON (*((EFI_REDFISH_MESSAGEREGISTRYFILECOLLECTION_CS **)((UINT8 *)InterpProp + sizeof (EFI_REST_RESOURCE_INTERPED_PROPERTY_HEADER))), ResoruceRaw);  
}

/**
  This function destory structure retunred in MessageRegistryFileCollectionToStruct.
...
EFI_STATUS
EFIAPI
MessageRegistryFileCollectionDestoryEfiStruct(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL   *This,
...
)
{
  ...
  MessageRegistryFileCollection = (EFI_REDFISH_MESSAGEREGISTRYFILECOLLECTION *)InterpProp;
  DestroyMessageRegistryFileCollection_CS (MessageRegistryFileCollection->MessageRegistryFileCollection); 
  return EFI_SUCCESS;
}

/**
  This function destory JSON raw text returned from MessageRegistryFileCollectionFromEfiStruct
...
EFI_STATUS
EFIAPI
MessageRegistryFileCollectionDestoryJson(
  IN EFI_REST_RESOURCE_INTERPRETER_PROTOCOL *This,
  IN EFI_REST_RESOURCE_TYPE_IDENTIFIER *RsrcIdentifier,  
  IN CHAR8 *ResoruceRaw
)
{
  ...
  return DestroyMessageRegistryFileCollection_Json (ResoruceRaw);
}
...
EFI_STATUS
EFIAPI
RedfishMessageRegistryFileCollectionEntryPoint (
...
EFI_STATUS
EFIAPI
RedfishMessageRegistryFileCollectionUnload (
  IN EFI_HANDLE ImageHandle
  )
{
